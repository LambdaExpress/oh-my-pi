...
- `reasoningEffortMap` — partial map from internal effort levels (`minimal|low|medium|high|xhigh|max`) to provider-specific strings (e.g. DeepSeek maps `xhigh -> "max"` and Fireworks GLM maps `minimal -> "none"`).
