...
3. **Managed (auto-learn) skills** (`omp-managed` provider) resolved dead-last with first-wins, so any same-named authored skill from any provider or custom directory takes precedence; within a project, project managed skills shadow user managed skills of the same name
...
7. `omp-managed` (priority 5) — auto-learn skills under project `.omp/managed-skills` and user `~/.omp/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); project managed skills shadow same-named user managed skills only within that project, while same-named authored skills still win
