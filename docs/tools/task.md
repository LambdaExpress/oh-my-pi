...
| `outputSchema` | JSON Schema (`object \| true \| string \| null` at the coarse wire-validation layer) | No | Invocation-specific structured-output contract. Omit it unless the call needs a custom contract; omission inherits agent frontmatter `output`, then the parent session schema. The literal `false` is excluded because it rejects every possible result. Item field in batch shape, top-level in flat shape. |
