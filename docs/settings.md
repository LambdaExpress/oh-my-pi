...
| --------------------------------- | ------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `defaultThinkingLevel`            | enum    | `high`  | `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `auto`. Override per run with `--thinking`; unsupported levels clamp to the selected model's highest available effort.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `display.foldToolRows`      | boolean | `false`          | Draw every activity row as a one-line summary instead of its full block: tool calls (cards, grouped reads, subagent tasks, custom/extension/MCP tools), TTSR rule injections (`TTSR: ts-set-map — …`), context injections (`Inject: AGENTS.md ×2 · Skills` for instruction files, rules, the skill index and memory the harness put in the model's context, with per-source previews under `ctrl+o`), skill prompts (`Inject: pixiv-imagepack-pipeline — …`), todo reminders, and late LSP diagnostics. Examples: `Eval: raw point-by-point inspection`, `Edit: ~/project/README.md +6 -5`, `Write: src/app.ts +42`, `Bash: bun test`. Device dispatches fold as the operation they ran (`ADB: shell logcat -d`, `ADB: pull /sdcard/shot.png`) rather than the `xd://` URL. File targets stay accent-colored, edits show `+N -M` and writes `+N`, and a status glyph marks running or failed calls. Consecutive folded rows drop the blank row between blocks (model replies keep theirs), so a tool run reads as one compact list. Toggle it mid-session with `app.tools.foldRows` (`Alt+Shift+O`). |
