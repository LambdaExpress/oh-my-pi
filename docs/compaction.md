...
Native V1 and V2 compaction require an official OpenAI route: provider `openai` on `api.openai.com`, or provider `openai-codex` under `https://chatgpt.com/backend-api`. An omitted base URL uses the official default. Proxies, Azure, custom providers, and non-official compaction endpoint overrides are excluded even when `remoteCompaction.enabled` is `true`. Unavailable native compaction advances to the next configured method; `/compact remote` falls back to a local summary. Explicit generic `compaction.remoteEndpoint` summarizers remain independent of this native-route restriction.
