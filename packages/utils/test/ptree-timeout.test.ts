...
`;
	...
	it.skipIf(process.platform !== "win32")(
		"does not leave a pipe-holding descendant alive after the Windows root exits",
		async () => {
			...
			try {
				...
				// Depending on the Windows process host, the descendant either keeps
				// the inherited pipe alive until our deadline or is reaped when the
				// root exits. Both paths must leave no live descendant behind.
				if (result.exitError) expect(result.exitError).toBeInstanceOf(TimeoutError);
				const deadline = Date.now() + 500;
