...
export interface CodexRequestOptions {
	/** User-facing effort, validated against the model ladder before wire mapping. */
...
/**
 * Validate a user-facing effort against the model's ladder, then apply the
 * compat/model effort map. Most efforts map 1:1; maps cover genuine wire
 * differences such as `minimal→none`. A mapped value outside the Codex wire
 * vocabulary is a broken effort map — fail loudly rather than silently
 * sending a different tier.
 */
