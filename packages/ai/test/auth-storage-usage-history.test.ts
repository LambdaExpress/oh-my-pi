...
describe("OpenCode Go usage via the upstream endpoint", () => {
	...
	it("carries the upstream resetsAt and duration through the OpenCode Go cap windows", async () => {
		...
		expect(fiveHour?.window?.resetsAt).toBe(Date.parse("2026-08-12T15:09:04.847Z"));
		expect(fiveHour?.window?.durationMs).toBe(5 * HOUR);
		const weekly = report.limits.find(limit => limit.id === "weekly");
		expect(weekly?.window?.resetsAt).toBe(Date.parse("2026-08-17T00:00:00.847Z"));
		expect(weekly?.window?.durationMs).toBe(7 * 24 * HOUR);
	...
	it("keeps the monthly window duration-less because the upstream anchors it to the subscription anniversary", async () => {
		const reports = await storage.fetchUsageReports();
		const report = reports?.find(candidate => candidate.provider === "opencode-go");
		const monthly = report?.limits.find(limit => limit.id === "monthly");
