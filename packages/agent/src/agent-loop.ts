			...
			try {
				...
				if (!completedToolExecution) {
					...
					try {
						...
						// The race can settle on abort while the abandoned execution is
						// still in flight, so release the taken session here rather than
						// after it (upstream's post-await finally).
