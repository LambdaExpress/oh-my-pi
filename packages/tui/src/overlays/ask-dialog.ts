...
function displayOptionLabels(question: ExtensionAskDialogQuestion): string[] {
	...
	// The sentinel must match the label the fork actually renders for the "Other" row.
	return disambiguateDisplayLabels(badged, [t("Other (type your own)"), ...GUEST_ACTION_LABELS]);
}
...
export class AskDialogComponent implements Component {
	...
	#expandHint(): string {
		...
		return ` · ${expandKeyHint()} ${t(this.#expanded ? "collapse" : "expand")}`;
	}
	...
	#footerHintText(indicator: string): string {
		...
		const enterAction = this.#questions.length > 1 ? t("next") : t("submit");
		const action = question?.multi
			? `${t("Space toggle")} · ${t("Enter {action}", { action: enterAction })} · n ${t("note")}`
		...
		if (this.#questionCanPage && indicator) {
			...
			const pagedAction = question?.multi
				...
				: t("Enter select");
			return `${pagedAction} · ↑/↓${tabs} · ${cancel}${expand} · ${pageKeysLabel()} ${indicator}`;
		}
		const scroll = indicator ? ` ${indicator} ${t("scroll")} ·` : "";
		return `${action} · ↑/↓ ${t("move")}${tabs} ·${scroll} ${cancel}${expand}`;
	}
