...
export type SessionTreeEntry = { id: string; parentId: string | null } & (
	...
	| { type: "title_change"; title: string; source: string }
	| { type: "mode_change"; mode: string }
	| { type: "credential_pin"; provider: string }
	// Local branch: SSH host-config edits persist as session entries too.
	| { type: "ssh_config_change"; operation: "upsert" | "delete"; name: string; config?: unknown }
	| { type: "ttsr_injection"; injectedRules: string[] }
	| { type: "session_init"; task: string }
	| { type: "reset_boundary" }
);
...
import { OverlayPanel, PanelDivider, row } from "../chrome/overlay-box";
...
function isBookkeepingEntry(entry: SessionTreeEntry): boolean {
	switch (entry.type) {
		case "label":
		case "custom":
		case "model_change":
...
class TreeList implements Component {
	...
	setHoverIndex(index: number | null): void {
		if (index === null || index < 0 || index >= this.#tree.rows.length) {
	...
	clickItem(index: number): void {
		const selected = this.#tree.rows[index];
		if (!selected) return;
		this.#tree.setSelectionIndex(index);
		this.onSelect?.(selected.item.entry.id, { summarize: false });
	}
	...
	render(width: number): readonly string[] {
		...
		if (this.#tree.rows.length === 0) {
			...
			if (totalCount === 0) {
				...
				lines.push(truncateToWidth(theme.fg("muted", t("Press Alt+A to show all, Alt+D for default")), width));
				lines.push(truncateToWidth(theme.fg("muted", `(0/${totalCount})${this.#getFilterLabel()}`), width));
			}
			return lines;
		}
		...
		const state = this.#tree.renderWithState(width);
		const renderedLines = [...state.lines];
		for (let i = 0; i < renderedLines.length; i++) this.#hitRows[i] = state.windowStart + i;
		const hovered = this.#hoveredIndex;
		if (hovered !== null && hovered !== this.#tree.selectionIndex) {
			const hoveredLine = hovered - state.windowStart;
			if (hoveredLine >= 0 && hoveredLine < renderedLines.length) {
				renderedLines[hoveredLine] = theme.bg("selectedBg", renderedLines[hoveredLine]!);
			}
		}
