...
export function formatMetrics(metrics: AgentMetrics): string {
	return formatMetricRow(
		[
			...
			{ value: formatMetricDuration(metrics) ?? `${t("time")} —` },
			{ value: `${formatNumber(metrics.requests)} ${t("req")}` },
			{ value: `${formatNumber(metrics.tools)} ${t("tools")}` },
			{ value: `${formatNumber(metrics.tokens)} ${t("tok")}` },
		],
