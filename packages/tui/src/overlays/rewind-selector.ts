...
export class RewindSelectorComponent implements Component {
		...
		if (data.startsWith("\x1b[<")) {
			routeSgrMouseInput(data, event => {
				if (event.wheel !== null) {
					...
					const hadHover = this.#hoveredTarget !== undefined;
					this.#hoveredTarget = undefined;
					if (this.#browser.scroll(event.wheel * 3) || hadHover) this.deps.requestRender();
				} else if (event.motion || event.leftClick) {
					const point = this.#browser.toContentPoint(event.row, event.col);
					const target = point
	...
	#renderStrip(mainRows: (readonly string[])[], columns: SiblingColumn[], contentWidth: number): ComposedColumn {
		...
		for (let index = 0; index < suffixRows.length; index++) {
			mainRows[anchor.start + index] = suffixRows[index]!;
		}
