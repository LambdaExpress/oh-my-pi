...
export class PluginListComponent extends OverlayPanel {
	...
	constructor(
		...
		if (entries.length === 0) {
			...
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("dim", t("Install npm plugins:        omp plugin install <package>")), 0, 0));
			this.addChild(
				new Text(theme.fg("dim", t("Install marketplace plugins: omp plugin install <name>@<marketplace>")), 0, 0),
			);
			this.addChild(new Spacer(1));
...
export class PluginDetailComponent extends OverlayPanel {
	...
	async #rebuild(): Promise<void> {
		...
		this.#settingsList = new SettingsFormField({
			...
			hint: t("Enter to edit · Esc to go back"),
...
export class MarketplacePluginDetailComponent extends OverlayPanel {
	...
	#render(runtimePlugin: InstalledPlugin | undefined, configItems: SettingItem[]): void {
		...
		if (entry?.gitCommitSha) {
			summary.push(new Text(theme.fg("dim", `${t("git sha")}       ${entry.gitCommitSha}`), 0, 0));
		}
...
function createConfigEnumPanel(
	...
	const field = new SelectFormField({
		...
		hint: t("Enter to select · Esc to cancel"),
		onSubmit: onSelect,
		onCancel,
		requestRender,
	});
...
function createConfigInputPanel(
	...
	let typeHint = `${t("Type")}: ${schema.type}`;
	...
	const field = new TextFormField({
		...
		empty: "cancel",
		hint: t("Enter to save · Esc to cancel"),
		onSubmit,
		onCancel,
		requestRender,
	});
