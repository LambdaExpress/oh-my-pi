...
function createSettingsTextField(
	...
	return new TextFormField({
		...
		empty: "submit",
		hint: t("  Enter to save · Esc to cancel · Clear field to unset"),
		onSubmit,
		onCancel,
		requestRender,
	});
...
function createSettingsSelectField(
	...
	return new SelectFormField({
		...
		onSubmit: onSelect,
		onCancel,
		hint: t("  Enter to select · Esc to go back"),
		footer,
		requestRender,
	});
}
...
class ProviderLimitsSubmenu extends Container {
	...
	#showProviderList(): void {
		...
		this.#listField = new SelectFormField({
			theme: formTheme,
			label: t("Max In-Flight Requests"),
			description: t(
			...
			maxVisible: 12,
			selectTheme: getSelectListTheme(),
			hint: t("  Enter to edit provider · Esc to go back"),
			onSubmit: value => {
	...
	#showProviderEditor(provider: string): void {
		...
		this.addChild(
			new TextFormField({
				theme: formTheme,
				label: t("Max In-Flight Requests: {provider}", { provider }),
				description: t(
				...
				validate: value => {
					...
					if (!Number.isFinite(limit) || limit <= 0) return t("Limit must be a positive number.");
					return undefined;
				},
				onSubmit: value => {
					const next = { ...limits };
...
function translatedLabel(path: string, fallback: string): string {
	switch (path) {
		...
		case "advisor.maxNotesPerUpdate":
			return t("Advisor Max Notes Per Update");
		...
		case "bash.allowCompoundCommands":
			return t("Allow Compound Commands");
		...
		case "browser.enabled":
			return t("Browser");
		case "browser.freezeOnTurnEnd":
			return t("Freeze Browser Tabs On Turn End");
		case "browser.headless":
			return t("Headless Browser");
		case "browser.idleCloseSec":
			return t("Browser Idle Close Timeout");
		case "browser.relay":
		...
		case "compaction.experimentalContextManagement":
			return t("Notes-backed context windows (experimental)");
		...
		case "compaction.methodOrder":
			return t("Compaction Method Order");
		...
		case "composer.recallClearedDrafts":
			return t("Recall Cleared Drafts");
		case "composer.shape":
			return t("Composer Shape");
		...
		case "edit.autoRepair.enabled":
			return t("Auto-Repair Parse Regressions");
		case "edit.blackbox.enabled":
			return t("Record Parse Regressions");
		...
		case "edit.mode":
			return t("Edit Mode");
		case "edit.recoverInlineEdits":
			return t("Recover Inline Edit Payloads");
		...
		case "eval.autoBackground.enabled":
			return t("Eval Auto-Background");
		case "eval.js":
			return t("JavaScript Eval Backend");
		case "eval.py":
			return t("Python Eval Backend");
		case "eval.workpool.freshAgents":
			return t("Fresh Workpool Agents");
		case "eval.tools.enabled":
			return t("Eval-Defined Tools");
		case "exa.searchDelayMs":
			return t("Exa Search Delay");
		case "exa.enabled":
			return t("Exa");
		case "extensionHandlers.toolCallTimeoutMs":
			return t("Tool Call Handler Timeout (ms)");
		case "extendedContextWindow":
			return t("Extended Context Window");
		case "extendedContext":
			return t("Extended Context");
		...
		case "images.urls.sshRemotePort":
			return t("Image URL SSH Remote Port");
		case "images.urls.sshTarget":
			return t("Image URL SSH Target");
		case "images.urls.bindHost":
			return t("Image URL Bind Host");
		case "images.urls.ttlHours":
			return t("Image URL Lifetime (hours)");
		case "images.urls.publicBaseUrl":
			return t("Image URL Public Base");
		case "images.urls.command":
			return t("Image Upload Command");
		case "images.urls.backends":
			return t("Image URL Backends");
		case "images.urls.enabled":
			return t("Serve Images as URLs");
		...
		case "live.voice":
			return t("Live Voice");
		case "loop.conditionTimeoutMs":
			return t("Loop Condition Timeout (ms)");
		case "loop.mode":
			return t("Loop Mode");
		...
		case "plan.autosaveDir":
			return t("Autosave Directory");
		case "plan.autosave":
			return t("Autosave Plans");
		...
		case "providers.cacheRetention":
			return t("Prompt Cache Retention");
		...
		case "providers.openai-codex.codeModeDirectTools":
			return t("Codex Code Mode Direct Tools");
		case "providers.openai-codex.codeMode":
			return t("Codex Code Mode");
		...
		case "sharpshooter.model":
			return t("Sharpshooter Model");
		...
		case "skillful":
			return t("List Skills in Prompt");
		...
		case "spelling.autocorrect":
			return t("Autocorrect (macOS)");
		case "spelling.autocomplete":
			return t("Word Autocomplete (macOS)");
		case "spelling.typoDetection":
			return t("Typo Detection (macOS)");
		...
		case "statusLine.contextLine":
			return t("Context-Reactive Line");
		...
		case "tools.speculativeExecution.maxInFlight":
			return t("Speculative Execution Concurrency");
		case "tools.speculativeExecution.enabled":
			return t("Experimental Speculative Execution");
		case "tools.xdev":
			return t("xd:// Tools");
		case "tools.xdevDocs":
		...
		case "tui.mouse":
			return t("Mouse Click-to-Focus");
		case "tui.reactions":
			return t("Agent Reactions");
		case "tui.renderMermaid":
			return t("Render Mermaid Diagrams");
		case "tui.resizeScrollback":
			return t("Resize Scrollback");
		case "tui.textSizing":
		...
		case "update.channel":
			return t("Update Channel");
		case "tui.vimModeDisplay":
			return t("Vim Mode Indicator");
		case "tui.vimMode":
			return t("Vim Editing Mode");
		case "vault.enabled":
		...
		case "worktree.cleanSource":
			return t("Clean Source Checkout on /wt");
		case "worktree.clone":
			return t("Clone Checkout into Worktrees");
		default:
			return fallback;
	}
}

function translatedDescription(path: string, fallback: string): string {
	switch (path) {
		case "advisor.enabled":
			return t(
		...
		case "composer.shape":
			return t("Visual layout of the input editor and status line");
		case "computer.display":
		...
		case "display.showTurnTime":
			return t("Show the total prompt-to-yield time (including tool calls) on assistant message usage rows");
		case "display.smoothStreaming":
		...
		case "edit.blackbox.enabled":
			return t("Append full before/after source when an edit introduces an AST parse failure");
		case "edit.blockAutoGenerated":
		...
		case "edit.recoverInlineEdits":
			return t("Execute edit payloads the model emits as plain text by converting them into edit tool calls");
		case "edit.streamingAbort":
		...
		case "eval.autoBackground.enabled":
			return t("Automatically background long-running eval cells and deliver the result later");
		case "eval.js":
		...
		case "eval.workpool.freshAgents":
			return t("Spawn a new subagent for every workpool item instead of reusing workers or batching queued items");
		case "eval.tools.enabled":
			return t(
		...
		case "images.urls.sshRemotePort":
			return t("Remote listen port of the ssh reverse forward that your web server proxies to");
		case "images.urls.sshTarget":
			return t("user@host destination for the ssh reverse forward");
		case "images.urls.bindHost":
			return t("Host the blob server binds to; loopback for tunnels, 0.0.0.0 for direct serving");
		case "images.urls.ttlHours":
			return t(
		...
		case "images.urls.publicBaseUrl":
			return t("Externally reachable base URL fronting the blob server (required for ssh, optional for direct)");
		case "images.urls.command":
			return t(
		...
		case "images.urls.backends":
			return t("Ordered destinations tried when publishing images for provider access");
		case "images.urls.enabled":
			return t(
		...
		case "plan.autosave":
			return t("Automatically save approved plans to disk when plan mode completes");
		case "plan.defaultOnStartup":
		...
		case "sharpshooter.model":
			return t("Model selector for extraction/consolidation, empty = smol role");
		case "shellMinimizer.enabled":
		...
		case "spelling.autocorrect":
			return t("Apply confident macOS spelling corrections after completed words");
		case "spelling.autocomplete":
			return t("Show macOS dictionary word completions as inline hints accepted with Tab");
		case "spelling.typoDetection":
			return t("Mark misspelled prompt words with the active macOS dictionaries");
		case "startup.changelogMode":
		...
		case "statusLine.contextLine":
			return t("How the line between the left and right segments reflects context usage (box composer only)");
		case "statusLine.preset":
		...
		case "tools.speculativeExecution.maxInFlight":
			return t("Maximum number of validated local reads allowed to run before normal dispatch.");
		case "tools.speculativeExecution.enabled":
			return t(
		...
		case "tui.reactions":
			return t("Invite the agent to react to your message with an emoji badge on its bubble");
		case "tui.renderMermaid":
			return t("Render Mermaid fenced code blocks as ASCII diagrams");
		case "tui.resizeScrollback":
			return t("How a settled terminal resize refreshes transcript rows retained in terminal scrollback");
		case "tui.textSizing":
			return t(
		...
		case "update.channel":
			return t("Update channel used by omp update and the startup update check");
		case "tui.vimModeDisplay":
			return t("How the current Vim mode appears in the status line");
		case "tui.vimMode":
			return t(
...
function translatedOptions(path: string, options: ReadonlyArray<SubmenuOption>): ReadonlyArray<SubmenuOption> {
	switch (path) {
		case "advisor.immuneTurns":
			return [
...
export class SettingsSelectorComponent implements Component {
	...
	#setSearchQuery(query: string): void {
		...
		for (const result of tabResults) {
			...
			items.push({
				id: `__tab:${result.tab}`,
				label: `${theme.symbol(meta.icon)} ${tabLabel(result.tab)}`,
				currentValue: "",
				heading: true,
			});
	...
	#createSubmenu(
		...
		if (def.path === "defaultThinkingLevel") {
			...
			// Shape labels/descriptions live in BUILTIN_COMPOSER_SHAPES (raw English
			// keys); extension-registered shapes pass through untranslated.
			options = getComposerShapeOptions().map(option => ({
				...option,
				label: t(option.label),
				description: option.description ? t(option.description) : undefined,
			}));
