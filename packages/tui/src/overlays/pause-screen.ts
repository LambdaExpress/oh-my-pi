...
export function renderPauseScreen(width: number, height: number, elapsedMs: number, sessionName?: string): string[] {
	...
	if (compact) {
		...
		content.push(centerLine(theme.bold(theme.fg("accent", `▌▌ ${t(TITLE)}`)), width).trimEnd());
		content.push("");
		content.push(
		...
		for (const line of BODY_LINES) {
			content.push(centerLine(theme.fg("muted", t(line)), width).trimEnd());
		}
		content.push("");
		content.push(
			centerLine(theme.fg("dim", t("paused for {time}", { time: formatClock(elapsedMs) })), width).trimEnd(),
...
export async function runPauseScreen(host: PauseScreenHost): Promise<void> {
	...
	try {
		...
		if (heldMs !== undefined) {
			host.showStatus(
				t("Resumed after {duration} — agents are running again.", { duration: formatCoarseDuration(heldMs) }),
			);
		}
	}
}
