	...
	implements SelectListMouseTarget
{
	...
	#renderTable(width: number, termHeight: number): string[] {
		...
		if (isSplit) {
			...
			topLine = topBorder(width, t("Agent Hub · {id}", { id: selected.id }));
		} else {
			topLine = topBorder(width, t("Agent Hub"));
		}
	...
	#renderDetailPanel(
		...
		section(t("Lineage"));
		add(
			`${t("Spawned by {parent}", { parent: sanitizeDisplaySingleLine(ref.parentId ?? MAIN_AGENT_ID) })}${
