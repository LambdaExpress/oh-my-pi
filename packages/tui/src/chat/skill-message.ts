...
import { fileHyperlink, truncateToWidth } from "../render";
...
export class SkillMessageComponent extends Container {
	...
	// `display.foldToolRows`: one `Inject:` row instead of the skill card.
	...
	#rebuild(): void {
		...
		if (!leading) {
			this.#disclosure = new Disclosure({
				...
				body: () => new SkillCallout([new Text(this.#header(name, details), 0, 0), ...this.#promptSection(bubble)]),
				expanded,
			});
		...
		const children: Component[] = [new Text(this.#header(name, details), 0, 0)];
	...
	#header(name: string, details: SkillPromptDetails | undefined): string {
		const chip = skillChipStyle(name, bubbleReset());
		// The skill prompt is context the harness injects, so its header carries the
		// same `Inject` label the folded row and the context-injection notice use.
		// The name itself stays the SKILL.md chip: the path is the link target, never
		// spelled out beside it.
		const tag = theme.fg("customMessageLabel", theme.bold(`${theme.icon.extensionSkill} ${t("Inject")}`));
		const parts = [`${tag} ${details?.path ? fileHyperlink(details.path, chip, { line: 1 }) : chip}`];
	...
	#promptSection(bubble: UserBubbleOptions): Component[] {
		...
		return [
			new Spacer(1),
			new Text(theme.fg("muted", t("prompt")), 0, 0),
			new Spacer(1),
