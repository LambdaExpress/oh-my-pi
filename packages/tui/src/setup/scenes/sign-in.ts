...
export class SignInTab implements SetupTab {
	...
	render(width: number, maxLines?: number): readonly string[] {
		...
		if (this.#loggingInProvider === undefined && (maxLines === undefined || maxLines >= 17 + 2)) {
			intro = new Container();
			intro.addChild(
				new Text(theme.fg("muted", t("Pick a provider to sign in — you can connect more than one.")), 0, 0),
			);
		...
		if (this.#authUrl) {
			...
			if (this.#authLaunchUrl) {
				tail.addChild(
					new Text(theme.fg("dim", `${t("Local shortcut (this machine only)")}: ${this.#authLaunchUrl}`), 0, 0),
				);
			}
		}
		if (this.#prompt) {
		...
		if (this.#loggingInProvider) {
			...
			this.#step.setHeading(
				new Text(theme.bold(t("Signing in to {provider}", { provider: this.#loggingInProvider })), 0, 0),
			);
	...
	async #login(providerId: string): Promise<void> {
		...
		try {
			...
			this.#statusLines = [
				theme.fg("success", `${theme.status.success} ${t("Signed in to {provider}", { provider: providerId })}`),
				theme.fg("dim", t("Credentials saved to {path}", { path: getAgentDbPath() })),
			];
			this.#authUrl = undefined;
			this.#authLaunchUrl = undefined;
			...
			if (this.#loginAbort?.signal.aborted) {
				...
				this.#statusLines = [
					theme.fg("error", `${t("Login failed")}: ${message}`),
