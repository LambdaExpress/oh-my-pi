...
interface WriteRenderArgs {
	...
	content?: unknown;
	/**
	 * Raw tool-call JSON prefix carried while the args still stream. Parsed args
	 * lag it by up to one throttled parse window, so the renderer prefers it.
	 */
...
/**
 * Tail-window start (a logical line index) for the collapsed streaming preview,
 * measured in WRAPPED rows: one long CJK or HTML line occupies several terminal
 * rows inside the frame, so a logical-line cap would let the collapsed block
 * outgrow the viewport it is supposed to fit. The last line always stays
 * visible — it is the live streaming edge — and the gutter is charged for too,
 * since it renders on the same wrapped rows.
 */
function streamingTailWindowStart(
	lines: readonly string[],
	totalLines: number,
	contentWidth: number | undefined,
): number {
	const logicalCap = Math.max(0, lines.length - PREVIEW_LIMITS.EXPANDED_LINES);
	if (lines.length <= 1 || contentWidth === undefined || contentWidth <= 0) return logicalCap;
	const gutter = " ".repeat(Math.max(WRITE_GUTTER_MIN_WIDTH, String(totalLines).length) + 1);
	let rows = 0;
	let start = lines.length;
	while (start > 0) {
		const rowCount = Math.max(1, wrapTextWithAnsi(`${gutter}${lines[start - 1] ?? ""}`, contentWidth).length);
		if (rows + rowCount > PREVIEW_LIMITS.EXPANDED_LINES) break;
		rows += rowCount;
		start--;
	}
	// The trailing line is the live streaming edge: keep it even when it alone
	// overflows the row budget, so a streaming write never blanks out.
	if (start === lines.length && lines.length > 1) start = lines.length - 1;
	return start;
}
...
function formatStreamingContent(
	...
	if (!content) return "";
	// The row-budgeted window depends on the inner width, so it keys the memo.
	const cacheSalt = `${language ?? ""}|${contentWidth ?? 0}`;
	const bodyText = cachedRenderedString(cache, uiTheme, expanded, cacheSalt, content, () => {
		const state = updateStreamingPreview(streamKey, content, language, uiTheme, argsComplete === true);
		let totalLines: number;
		let allLines: string[];
		let highlightSlice = false;
		if (state) {
			...
			allLines = [...state.highlightedLines, trailingLine];
		} else {
		...
		const startIndex = expanded ? 0 : streamingTailWindowStart(allLines, totalLines, contentWidth);
		const visibleLines = highlightSlice
			? highlightCode(allLines.slice(startIndex).join("\n"), language)
		...
		let text = "\n\n";
		if (hidden > 0) {
			// Local marker format, shared with `capPreviewLines`: `… N earlier lines`.
	...
	return `${bodyText}${spinner}${uiTheme.fg("dim", t("… (streaming)"))}`;
}
...
export const writeToolRenderer = {
	...
	renderCall(
		...
		// The raw prefix wins whenever it carries a field: a mid-stream rebuild
		// (theme change, settings, focus replay) recreates this component while
		// args still stream, and the provider-parsed values lag the buffer — a
		// preview reading them alone freezes at the last full parse.
		const partialPath =
		...
		// Render NOTHING until one of the payload fields arrives. A path-only
		// stream may still provably be an xd:// device; a content-first stream
		// renders under the "…" path placeholder until the path lands. A
		// present-but-malformed path (array/object from a bad provider parse) is
		// definitively not xd:// — it falls through to the legacy frame.
		...
		if (rawPath && couldBecomeXdUrl(rawPath)) {
			...
			const pathSettled = partialContent !== undefined || args.content !== undefined;
			if (!xdev?.name || !pathSettled) return undefined;
			if (isResolutionDeviceName(xdev.name)) return renderResolutionDeviceCall(xdev.name, rawContent, uiTheme);
			if (xdev.name === REPORT_ISSUE_DEVICE_NAME) return renderReportIssueDeviceCall(rawContent, uiTheme);
			return renderXdevCall(xdev.name, rawContent, options, uiTheme, options.renderContext?.resolveXdevMounted);
		}
		...
		const content = typeof rawContent === "string" ? rawContent : normalizeDisplayText(rawContent);
		...
		return framedToolCard(uiTheme, ({ contentWidth }) => {
			const body = content
				? formatStreamingContent(
						...
						// Collapsed previews budget by wrapped rows, so the cap needs
						// the same inner width the card wraps the body at.
						contentWidth,
					)
				: "";
