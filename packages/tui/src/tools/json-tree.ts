...
export function formatOutputInline(data: unknown, maxWidth = 80): string {
	...
	if (typeof data !== "object") return t("Output: {value}", { value: formatScalar(data, 60, options) });
	if (Array.isArray(data)) {
		if (data.length === 0) return t("Output: []");
		return t("Output: [{count} items] {preview}{ellipsis}", {
			count: data.length,
			preview: formatScalar(data[0], 40, options),
			ellipsis: data.length > 1 ? "…" : "",
		});
	}
	return t("Output: {value}", {
		value: formatArgsInline(data as Record<string, unknown>, maxWidth - "Output: ".length, options) || "{}",
	});
}
	...
	const tree = new TreeView<JsonTreeNode, number>({
		...
		renderRow: item => {
			...
			const label = theme.fg(
				"muted",
				displayKey || (item.kind === "array" ? t("array") : item.kind === "object" ? t("object") : t("value")),
			);
			if (item.kind === "placeholder") {
				...
				if (multilineStrings && sanitized.includes("\n")) {
					...
					if (sourceLines.length > displayedCount) {
						...
						scalarLines.push(
							`${indent}${theme.fg("dim", `${t(" …({count} more lines)", { count: sourceLines.length - displayedCount })}"`)}`,
						);
					} else {
