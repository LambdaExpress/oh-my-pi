...
export interface ShellRendererConfig<TArgs> {
	...
	/**
	 * Local hook: a caller with its own language (pwsh) highlights its command
	 * lines itself instead of running the bash highlighter over the projection.
	 */
	formatCommandLines?: (args: TArgs | undefined, uiTheme: Theme) => string[];
	/** Symbol shown on the success header; defaults to the bash tool symbol. */
