		...
		return {
			...
			sections: [
				...(queryPreview
					? [
							{
								content: [`${theme.fg("muted", t("Query:"))} ${theme.fg("text", queryPreview)}`],
							},
						]
					: []),
				{
				...
				},
				{
					...
					content: sourceTree.length > 0 ? sourceTree : [theme.fg("muted", t("No sources returned"))],
				},
				{ label: theme.fg("toolTitle", t("Metadata")), content: metaLines },
			],
		};
	});
}
