...
export interface InternalUrlCompletionHost {
	...
	resolveCompletions(
		scheme: string,
		query: string,
		context: InternalUrlCompletionContext,
...
/** SSH host fields the completion host needs to offer `ssh://` candidates. */
export interface InternalUrlSshHost {
	name: string;
	host: string;
	username?: string;
	port?: number;
...
export interface InternalUrlCallerContext {
	...
	sessionId?: string;
	/**
	 * Lazy accessor for the calling session's effective SSH hosts, read only
	 * when an `ssh://` token is being completed (`getSessionSshHosts` is async).
	 */
	sshHosts?: () => Promise<readonly InternalUrlSshHost[]>;
}

/** Context handed to the completion host: caller binding plus resolved lazy capabilities. */
export type InternalUrlCompletionContext = Omit<InternalUrlCallerContext, "sshHosts"> & {
	sshHosts?: readonly InternalUrlSshHost[];
	signal?: AbortSignal;
};
...
export async function getInternalUrlSuggestions(
	...
	const { sshHosts: sshHostsAccessor, ...caller } = getCaller?.() ?? {};
	// Only `ssh://` completions need session hosts; every other scheme leaves the
	// accessor untouched so the lazy lookup never runs.
	const sshHosts = ctx.scheme === "ssh" ? await sshHostsAccessor?.() : undefined;

	const candidates = await completionHost?.resolveCompletions(ctx.scheme, ctx.query, {
		...(cwd === undefined ? {} : { cwd }),
		...caller,
		...(signal ? { signal } : {}),
		...(sshHosts && sshHosts.length > 0 ? { sshHosts } : {}),
	});
