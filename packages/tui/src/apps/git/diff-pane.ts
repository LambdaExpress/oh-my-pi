...
export class DiffPane {
	...
	#renderAsset(width: number, height: number): string[] {
		...
		for (let index = 0; index < height; index++) {
			const leftSource =
				index === 0
					? centerLine(theme.bold(this.#assetTitle(t("Before"), asset.old)), leftWidth).trimEnd()
					: (oldLines[index - 1] ?? "");
			const rightSource =
				index === 0
					? centerLine(theme.bold(this.#assetTitle(t("After"), asset.new)), rightWidth).trimEnd()
					: (newLines[index - 1] ?? "");
	...
	#renderStreaming(width: number, height: number): string[] {
		...
		if (total === 0) {
			return Array.from({ length: height }, (_, index) =>
				index === Math.floor(height / 2) ? centerLine(theme.fg("dim", t("Streaming file…")), width).trimEnd() : "",
			);
		}
		this.#clampScroll();
