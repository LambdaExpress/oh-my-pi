...
export class CleanseBoardModel {
	...
	checkerFinished(check: CleanseCheckResult, durationMs: number): string {
		...
		const verdict =
			count === 0
				? fgOrPlain("success", t("clean"))
				: fgOrPlain("warning", t("{count} issue{s}", { count, s: count === 1 ? "" : "s" }));
			...
			if (!board.interactive) {
				...
				if (outcome.success) {
					...
					const message = truncateToWidth(
						sanitizeDisplaySingleLine(outcome.error ?? t("subagent failed"))
							.replace(/\s+/g, " ")
							.trim(),
						ERROR_WIDTH,
					);
...
function renderWaveHeader(
	...
	return `${fgOrPlain("accent", spinner)} ${t("Repairing [{bar}] {parts}", { bar, parts: parts.join(fgOrPlain("dim", " · ")) })}`;
}
...
function agentActivity(progress: AgentProgress | undefined): string {
	if (!progress) return fgOrPlain("dim", t("starting"));
	...
	return intent || fgOrPlain("dim", t("thinking"));
}
