...
function renderLive(
	...
	const header =
		...
		`${t("{count} live", { count: rows.size })} ${fgOrPlain("dim", "·")} ${t("{count} actions", { count: deepest })} ${fgOrPlain("dim", "·")} ` +
	...
	for (const [label, row] of rows) {
		...
		if (row.placement) parts.push(fgOrPlain("dim", t("cat@{placement}", { placement: row.placement })));
		lines.push(
			renderTableRow(
				[
...
function formatVerdict(report: IfBenchModelReport, meta: IfBenchBoardMeta): string {
	...
	return `${fgOrPlain("error", "✗")} ${body} ${fgOrPlain("dim", "·")} ${fgOrPlain(
		"error",
...
function failureDetail(report: IfBenchModelReport): string[] {
	...
	if (failed.failure === "provider")
		return [`    ${fgOrPlain("dim", t("provider"))} ${fgOrPlain("error", oneLine(failed.response))}`];
	return [
		`    ${fgOrPlain("dim", t("expected"))} <${failed.expected}>`,
		`    ${fgOrPlain("dim", t("actual  "))} ${oneLine(failed.response)}`,
	];
}
