...
interface FastTailRecipe {
	...
	readonly paragraphTokens?: readonly Token[]; // append-safe paragraph state for settled-prefix publication
...
const UNSAFE_APPENDABLE_TEXT_MARKUP = /[\t\r\n\\`*_[\]<>~&$]/;

/**
 * Whether append-only growth can only extend the paragraph's trailing text.
 * Closed code spans are immutable lexer tokens, so they cannot be reinterpreted
 * by later source. Text tokens stay conservative: any Markdown delimiter could
 * pair with a future append and restyle rows already written to scrollback.
 */
function isAppendSafeParagraph(text: string, tokens: readonly Token[]): boolean {
	const paragraph = tokens.length === 1 ? tokens[0] : undefined;
	if (paragraph?.type !== "paragraph" || paragraph.raw !== text || !paragraph.tokens?.length) return false;
	for (const inline of paragraph.tokens) {
		if (inline.type !== "codespan" && (inline.type !== "text" || UNSAFE_APPENDABLE_TEXT_MARKUP.test(inline.raw))) {
			return false;
		}
	}
	return true;
...
function appendToSafeParagraph(
	...
	const inlineTokens = paragraph.tokens.slice();
	for (const token of lexInlineTokens(delta)) {
		if (token.type !== "codespan" && (token.type !== "text" || UNSAFE_APPENDABLE_TEXT_MARKUP.test(token.raw))) {
			return undefined;
		}
		const previous = inlineTokens[inlineTokens.length - 1];
		if (previous?.type === "text" && token.type === "text") {
			inlineTokens[inlineTokens.length - 1] = {
				...previous,
				raw: previous.raw + token.raw,
	...
	const nextParagraph: Tokens.Paragraph = {
		...paragraph,
		type: "paragraph",
		raw: nextText,
		text: paragraph.text + delta,
		tokens: inlineTokens,
	};
...
export class Markdown implements Component {
	...
	setText(text: string): boolean {
		...
		if (!text.startsWith(this.#text)) {
			...
			// The previous frame's guard verdict cannot be reused because the
			// checked region may have changed anywhere.
	...
	render(width: number): readonly string[] {
		...
		if (
			...
			if (!this.#signatureEquals(signature, recipe.signature)) {
				...
				if (
					...
					const paragraphTokens = appendToSafeParagraph(
						recipe.paragraphTokens,
						recipe.source,
						normalizedText,
						deltaTabs,
					);
					const paddingRows = this.#paddingRowCount(signature.paddingY);
					const contentRowCount = fastResult.length - paddingRows * 2;
					this.#plainParagraphPrefix = paragraphTokens
						? this.#plainParagraphPrefixCandidate(normalizedText, paragraphTokens, contentRowCount, signature)
						: undefined;
					this.#cachedText = this.#text;
					this.#cachedWidth = width;
		...
		const paragraphTokens = isAppendSafeParagraph(normalizedText, tokens) ? tokens.slice() : undefined;
	...
	#plainParagraphPrefixCandidate(
		...
		if (!this.transientRenderCache || contentRowCount <= 1 || !isAppendSafeParagraph(text, tokens)) {
			return undefined;
		}
		return {
			text,
