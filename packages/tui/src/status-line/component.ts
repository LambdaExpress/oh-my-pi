...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	refreshUsageInBackground(): void {
		...
		this.#usageStartTimer = setTimeout(() => {
			this.#usageStartTimer = null;
			void this.#runUsageRefresh(session, usageRefreshGeneration);
		}, STATUS_USAGE_START_DELAY_MS);
	}

	async #runUsageRefresh(session: TSession, usageRefreshGeneration: number): Promise<void> {
		...
		try {
			...
			if (signal.aborted && reportsPromise) {
				this.#observeLateUsageRefresh(session, reportsPromise, usageRefreshGeneration, sequence);
			}
		} finally {
	...
	#applyUsageRefreshReports(
		...
		if (sequence < this.#latestAppliedUsageRefreshSequence) {
			return;
		}
	...
	#observeLateUsageRefresh(
		...
		void reportsPromise
			.then(reports => {
				this.#applyUsageRefreshReports(session, reports, usageRefreshGeneration, sequence);
			})
			.catch(() => {
				if (this.#disposed || this.session !== session || this.#usageRefreshGeneration !== usageRefreshGeneration)
					return;
	...
	#normalizeUsageReports(
		...
		for (const report of reports) {
			...
			for (const limit of limits) {
				...
				const rawPlanType =
					provider === "openai-codex" || provider === "zai" ? usageReport.metadata?.planType : undefined;
	...
	#buildSegmentContext(
		...
		return {
			...
			usageFetchedAt: this.#usageFetchedAt,
		};
	}
