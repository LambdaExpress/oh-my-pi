...
export function collapseEffortVariants<TSpec extends VariantSpecLike>(
	...
	for (const family of table.families) {
		...
		if (existingCollapsed) {
			...
			// while curated families still refresh stale capability/routing metadata.
			// Reconcile the preferred default against members this account actually
			// advertised after retired targets have been removed.
			replacement.set(
				family.id,
				reconcileDefaultMember(
					refreshCollapsedThinking(reconciled as TSpec, family, retired),
					family,
					new Set(rawPresent),
				),
			);
