...
// kimi-for-coding[-highspeed] -> 32768, #6711); v11 invalidates rows written
// by either pre-merge v10 lineage (reset-branch rows may persist headers,
// upstream rows predate the distinct max-effort route for Devin GPT-5.6) and
// rows that may persist derived computer-use headers, recording which model
// ids lost headers and which cannot be rebuilt from static inputs, so the
// manager can restore the safe subset or refetch dynamic-only headers.
