...
describe("MemoryProtocolHandler", () => {
	...
	(resolvePwshExecutable() ? it : it.skip)(
		"PowerShell cannot read a peer's memory after its caller exits",
		async () => {
			await withMemoryFixture(async ({ cwd, memoryRoot }) => {
				await Bun.write(path.join(memoryRoot, "memory_summary.md"), "private peer memory");
				const tool = new PwshTool({
					cwd,
					hasUI: false,
					settings: Settings.isolated({ "memory.backend": "local" }),
					getSessionId: () => "departed-session",
					getSessionFile: () => null,
					getSessionSpawns: () => null,
				});
				const result = await tool.execute("departed-memory", {
					script: "$ErrorActionPreference = 'Stop'; Get-Content memory://root",
				});
				...
				expect(
					result.content
						...
						.map(item => item.text)
						.join("\n"),
				).not.toContain("private peer memory");
			});
		},
	);
...
describe("MemoryProtocolHandler — mnemopi bridge (issue #4443)", () => {
	...
	it("rejects fact update and invalidation as not_editable without changing the fact (issue #4725)", async () => {
		await withMnemopiSession(async ({ state }) => {
			const beam = state.memory.beam;
			beam.db
				.prepare(
	...
	it("deletes a fact when forgetting its id (issue #4725)", async () => {
		await withMnemopiSession(async ({ state }) => {
			const beam = state.memory.beam;
			beam.db
				.prepare(
				...
				.run("fact-forget", beam.sessionId, "service", "uses", "postgres", "2026-07-01T00:00:00.000Z", 0.9);

			expect(state.editScopedMemory("forget", "fact-forget")).toMatchObject({
				status: "deleted",
				store: "fact",
			});
			expect(beam.db.prepare("SELECT fact_id FROM facts WHERE fact_id = ?").get("fact-forget")).toBeNull();
		});
	});
