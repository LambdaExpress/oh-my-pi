...
import { type GhToolDetails, githubToolRenderer } from "@oh-my-pi/pi-tui/tools/github";
...
// and the native VCS implementation used by the GitHub tool. `/dev/null` is
...
`,
	...
	it("streams the complete job list, step progress, and recent available logs", async () => {
		...
		const runSpy = vi.spyOn(github, "run").mockImplementation(async (_cwd, args) => {
