...
describe("SkillMessageComponent", () => {
	...
	it("renders a leading invocation as a railed callout with the Inject label, meta, and the full multi-line body", () => {
		...
		// Every row carries the rail; the header carries the shared `Inject`
		// label with the skill name, not the raw `/skill:` token.
		...
		// The header label is the link to SKILL.md; the path itself is never spelled out.
		expect(lines.join("\n")).toContain(skillUri);
		...
		expect(rows.findIndex(row => row.includes("stage all"))).toBeGreaterThan(
			rows.findIndex(row => row.includes("Inject")),
		);
	...
	it("labels only the invoked skill; a second token the dispatcher ignored stays literal", () => {
		const component = new SkillMessageComponent(
			makeMessage({
				name: "atomic-commit",
				path: skillPath,
	...
	it("folds the injected prompt into one Inject row", () => {
		...
		expect(strip(component.render(80))).toContain(uiTheme.symbol("skill.rail"));
	});
});
