...
for (const mode of ["TUI", "headless"] as const) {
	describe(`/rename (${mode})`, () => {
		...
		it("discards a pending rename after switching away and back to the same session", async () => {
			...
			// SessionManager keys memory files by the raw sessionDir for fresh sessions but by
			// path.resolve(sessionFile) when loading, so the harness needs an already-absolute,
			// already-normalized dir; a POSIX-only literal like "/sessions" diverges on Windows.
			const sessionDir = path.join(os.tmpdir(), "omp-rename-switch-sessions");
			const source = SessionManager.create("/tmp/rename-switch", sessionDir, storage);
			const { session, sessionManager, execute, ctx, runtime } = createRuntime(mode, undefined, source);
			const output = mode === "TUI" ? vi.spyOn(ctx, "showStatus") : vi.spyOn(runtime, "output");
			await source.ensureOnDisk();
			await source.flush();
			const sourceFile = source.getSessionFile()!;
			const other = SessionManager.create("/tmp/rename-switch", sessionDir, storage);
...
describe("/rename slash command", () => {
	it("routes the title through the rename handler and saves the full command to history", async () => {
		const { session, ctx, execute } = createRuntime("TUI");
		const setText = vi.spyOn(ctx.editor, "setText");
		const handleRename = vi.spyOn(ctx, "handleRenameCommand");

		const handled = await execute("/rename my session");

		expect(handled).toBe(true);
		expect(setText).toHaveBeenCalledWith("");
		expect(handleRename).toHaveBeenCalledWith("my session");
		expect(session.sessionName).toBe("my session");
	});

	// The fork additionally pinned blank input to "Usage: /rename <title>". Upstream's rebuilt
	// handler (src/slash-commands/builtin-lifecycle.ts) trims the args and falls back to title
	// generation instead, and the `/rename (TUI|headless)` cases above already cover that
	// contract, so the usage assertion is intentionally not restored.
});
