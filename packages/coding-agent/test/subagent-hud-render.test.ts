...
describe("subagent HUD lines", () => {
	...
	it("lists sync and detached spawns alike", () => {
		// Supersedes the fork test "hides non-detached spawns: sync task calls and
		// eval agent() helpers": the v18.1.19 HUD rewrite dropped the `detached`
		// filter from isHudSubagent, so every active subagent is jump-listed.
	...
	it("collapses to a few rows with an expander by default", () => {
		// Supersedes the fork test "renders the first eight active detached
		// subagents and summarizes the rest": SUBAGENT_HUD_VISIBLE_LIMIT (8) and
		// its "N more running" summary were replaced by
		// SUBAGENT_HUD_COLLAPSED_LIMIT (3) plus the expander row asserted here;
		// the expanded counterpart is covered by the test above.
		const active = Array.from({ length: 10 }, (_, index) =>
			makeSession({
				id: `Worker${index}`,
				description: `job ${index}`,
...
describe("InteractiveMode subagent observer UI sync", () => {
	...
	it("advances the observer scope when AgentSession switches root sessions", async () => {
		await mode.init({ suppressWelcomeIntro: true });
		const previousScopeId = session.getAgentScopeId();
		vi.useFakeTimers();

		expect(await session.newSession()).toBe(true);
		const currentScopeId = session.getAgentScopeId();
	...
	it("applies the setting over a clicked expand override", async () => {
		...
		for (let index = 0; index < 5; index++) {
			eventBus.emit(
				TASK_SUBAGENT_LIFECYCLE_CHANNEL,
				makeLifecycle(`Override${index}`, index, `job ${index}`, undefined, session.getAgentScopeId()),
			);
		}
		await Promise.resolve();
