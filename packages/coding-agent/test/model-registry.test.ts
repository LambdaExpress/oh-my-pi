...
describe("ModelRegistry", () => {
	...
	describe("extended context", () => {
		...
		test("clamps cached Codex overrides before configured premium expansion", async () => {
			writeRawModelsJson({
				"openai-codex": {
					modelOverrides: {
						"gpt-6-astra": { contextWindow: 2_000_000 },
						"gpt-5.6-terra": { contextWindow: 3_000_000 },
					},
				},
			});
			const testSettings = Settings.isolated({ extendedContextWindow: "2M" });
			testSettings.set("extendedContext", true);
			const registry = new ModelRegistry(authStorage, modelsJsonPath, { settings: testSettings });
			const astra = registry.find("openai-codex", "gpt-6-astra");
			const terra = registry.find("openai-codex", "gpt-5.6-terra");
			if (!astra || !terra) throw new Error("Expected bundled Codex models");
			...
			writeModelCache(
				...
				[
					...
					{ ...terra, contextWindow: 272_000, maxContextWindow: 1_200_000 },
				],
				true,
				"",
			...
			await registry.reapplyModelPolicies();
			expect(registry.find("openai-codex", "gpt-6-astra")?.contextWindow).toBe(922_000);
			expect(registry.find("openai-codex", "gpt-5.6-terra")?.contextWindow).toBe(1_200_000);
		...
		test("reapplyModelPolicies keeps premium window floors within Codex server ceilings", async () => {
			const testSettings = Settings.isolated();
			testSettings.set("extendedContext", true);
			const registry = new ModelRegistry(authStorage, modelsJsonPath, { settings: testSettings });
			...
			testSettings.set("extendedContextWindow", "372K");
			await registry.reapplyModelPolicies();
			expect(registry.find("openai", "gpt-5.6-terra")?.contextWindow).toBe(1_050_000);
			...
			testSettings.set("extendedContextWindow", "2M");
			await registry.reapplyModelPolicies();
			expect(registry.find("openai", "gpt-5.6-terra")?.contextWindow).toBe(2_000_000);
			// Without a live or curated maximum, Codex retains the configured floor.
			expect(registry.find("openai-codex", "gpt-5.6-terra")?.contextWindow).toBe(2_000_000);
			const terra = registry.find("openai-codex", "gpt-5.6-terra");
			if (!terra) throw new Error("Expected bundled Codex Terra model");
			writeModelCache(
				"openai-codex",
				Date.now(),
				[{ ...terra, contextWindow: 272_000, maxContextWindow: 1_200_000 }],
		...
		test("explicit custom windows survive policy toggles within their provider ceilings", async () => {
			const testSettings = Settings.isolated({ extendedContextWindow: "2M" });
			writeRawModelsJson({
				"relay-provider": {
				...
				"openai-codex": {
					baseUrl: "https://codex.example.com/backend-api/codex",
					apiKey: "TEST_KEY",
					api: "openai-codex-responses",
					models: [
						{ id: "gpt-6-astra", contextWindow: 2_000_000 },
						{ id: "gpt-6-astra-wm", contextWindow: 400_000 },
					],
				},
				openai: {
					...providerConfig(
					...
					modelOverrides: { "gpt-6-astra": { contextWindow: 2_000_000 } },
				},
			});
			...
			expect(registry.find("openai-codex", "gpt-6-astra")?.contextWindow).toBe(922_000);
			expect(registry.find("openai-codex", "gpt-6-astra-wm")?.contextWindow).toBe(400_000);
			expect(registry.find("openai", "gpt-6-astra")?.contextWindow).toBe(2_000_000);
