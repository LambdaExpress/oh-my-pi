...
describe("getContextUsageLevel", () => {
	// 1M window: token thresholds (150k/270k/500k) win over percent thresholds.
	...
	// Small windows: percent thresholds (50/70/90) win because the token
	// thresholds exceed the window.
	it("uses percent thresholds for 128k windows", () => {
		expect(getContextUsageLevel(49, 128_000)).toBe("normal");
		expect(getContextUsageLevel(50, 128_000)).toBe("warning");
		expect(getContextUsageLevel(70, 128_000)).toBe("high");
		expect(getContextUsageLevel(90, 128_000)).toBe("error");
	});

	it("keeps unknown windows on percent thresholds", () => {
		expect(getContextUsageLevel(49, 0)).toBe("normal");
		expect(getContextUsageLevel(50, 0)).toBe("warning");
		expect(getContextUsageLevel(70, 0)).toBe("high");
