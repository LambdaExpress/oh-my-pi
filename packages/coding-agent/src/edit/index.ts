...
import {
	applyInternalEditWrite,
	type EditTargetContext,
	type EditTargetOperation,
	type EditVirtualResourceInput,
	isHostBackedEditTargetPath,
	type ResolvedEditVirtualResource,
...
type NativeVirtualEditSession = EditSession & {
	preloadVirtualResource(resource: EditVirtualResourceInput): Promise<void>;
};

type ResolvedInternalEditTarget = ResolvedEditVirtualResource["target"];

interface EditSessionState {
	native: NativeVirtualEditSession;
	rawInput: boolean;
	streamedArgs: string;
	pending: Promise<void>;
	finished: boolean;
	preloadError?: unknown;
	targetContext?: Promise<EditTargetContext>;
	preparedIntents: Set<string>;
	internalTargets: Map<string, ResolvedInternalEditTarget>;
}
...
export class EditTool implements AgentTool<TInput> {
	...
	openArgStream(init: AgentToolArgStreamInit): AgentToolArgStream {
		...
		return {
			push: delta => {
				...
				const inspection = this.#inspectSerialized(
					state.rawInput ? JSON.stringify({ input: state.streamedArgs }) : state.streamedArgs,
				);
				state.pending = state.pending.then(async () => {
					try {
						await this.#preloadInspection(state, inspection, undefined, false);
					} catch {
						// A partial path may not resolve yet. The next delta retries it,
						// and the complete payload is resolved strictly before apply.
					}
					state.native.push(delta);
				});
			},
			end: args => {
				const inspection =
					args === undefined
						? this.#inspectSerialized(
								state.rawInput ? JSON.stringify({ input: state.streamedArgs }) : state.streamedArgs,
							)
						: this.#inspect(args);
				state.pending = state.pending.then(async () => {
					try {
						await this.#preloadInspection(state, inspection, undefined, true);
						state.preloadError = undefined;
					} catch (error) {
						state.preloadError = error;
					}
					state.native.finish();
					state.finished = true;
				});
	...
	async execute(
		...
		await state.pending;
		try {
			await this.#preloadInspection(state, this.#inspect(params), signal, true);
			state.preloadError = undefined;
		} catch (error) {
			state.preloadError = error;
		}
		if (state.preloadError) {
			state.native.close();
			if (this.#sessions.get(toolCallId) === state) this.#sessions.delete(toolCallId);
			throw state.preloadError;
		}
		if (!streamed) state.native.setArgsJson(JSON.stringify(params));
		if (!state.finished) {
			state.native.finish();
			state.finished = true;
		}
		...
		let outcome;
		try {
			outcome = await state.native.apply(
				{ lspBatchId: batch?.id, lspFlush: batch?.flush ?? false },
				(_error, request) => this.#write(state, request, signal),
			);
		...
		for (const file of outcome.files) {
			...
			if (state.internalTargets.has(file.path) || (file.moveTo && state.internalTargets.has(file.moveTo))) continue;
	...
	#inspectSerialized(args: string): EditInspection {
		try {
			return editInspect(this.mode, args);
		} catch {
			return { paths: [], entries: [], fileOps: [] };
		}
	}

	async #targetContext(state: EditSessionState, signal?: AbortSignal): Promise<EditTargetContext> {
		state.targetContext ??= (async () => ({
			cwd: this.session.cwd,
			localProtocolOptions: this.session.localProtocolOptions,
			sshHosts: await this.session.getSessionSshHosts?.(),
		}))();
		const context = await state.targetContext;
	...
	async #preloadInspection(
		...
		const intents = new Map<
			string,
			{ path: string; op: EditTargetOperation; move?: string; enforcePlanMode?: boolean }
		>();
		for (const path of inspection.paths) intents.set(path, { path, op: "update" });
		for (const entry of inspection.entries) {
			if (!intents.has(entry.path)) intents.set(entry.path, { path: entry.path, op: "update" });
		}
		for (const fileOp of inspection.fileOps) {
			if (fileOp.kind === "delete") {
				intents.set(fileOp.path, { path: fileOp.path, op: "delete" });
			} else if (fileOp.kind === "move" && fileOp.to) {
				intents.set(fileOp.path, { path: fileOp.path, op: "update", move: fileOp.to });
				intents.set(fileOp.to, { path: fileOp.to, op: "create", enforcePlanMode: false });
			}
		}
		if (intents.size === 0) return;

		const context = await this.#targetContext(state, signal);
		const pending: Array<
			Promise<{
				intentKey: string;
				resolved: ResolvedEditVirtualResource | undefined;
			}>
		> = [];
		for (const intent of intents.values()) {
			const intentKey = JSON.stringify([intent.path, intent.op, intent.move ?? null]);
			if (!refresh && state.preparedIntents.has(intentKey)) continue;
			if (!isHostBackedEditTargetPath(intent.path)) {
				state.preparedIntents.add(intentKey);
				continue;
			}
			pending.push(
				resolveEditVirtualResource(this.session, intent.path, {
					op: intent.op,
					move: intent.move,
					signal,
					enforcePlanMode: intent.enforcePlanMode,
					context,
				}).then(resolved => ({ intentKey, resolved })),
			);
		}
		const resources = await Promise.all(pending);
		for (const { intentKey, resolved } of resources) {
			if (resolved) {
				await state.native.preloadVirtualResource(resolved.resource);
				state.internalTargets.set(resolved.target.canonicalPath, resolved.target);
			}
			state.preparedIntents.add(intentKey);
		}
	}
	...
	async #write(state: EditSessionState, request: EditWriteRequest, signal?: AbortSignal): Promise<EditWriteResponse> {
		const source = state.internalTargets.get(request.path);
		const destination = request.moveTo ? state.internalTargets.get(request.moveTo) : undefined;
		const sourceIsInternal = isHostBackedEditTargetPath(request.path);
		const destinationIsInternal = request.moveTo ? isHostBackedEditTargetPath(request.moveTo) : false;
		if (source || destination || sourceIsInternal || destinationIsInternal) {
			if (!source) {
				throw new ToolError("Remote/local edit moves are not supported", { path: request.displayPath });
			}
			if (request.op === "move" && !destination) {
				throw new ToolError("Remote/local edit moves are not supported", { path: request.displayPath });
			}
			const written = await applyInternalEditWrite(
				source,
				request,
				await this.#targetContext(state, signal),
				destination,
			);
			return { written };
		}
