...
const SETTING_HOOKS: Partial<Record<SettingPath, SettingHook<any>>> = {
	...
	// Local feature: the transcript draws every activity row as its one-line
	// summary, so the fold flag rides along with the other display preferences.
	"display.foldToolRows": value => {
		if (typeof value === "boolean") setChatTranscriptDisplayPreferences({ foldToolRows: value });
	},
...
export function resetSettingsForTest(): void {
	...
	// Persisted Settings instances own the process-wide AgentStorage singleton.
	// Close those SQLite handles with the settings reset so temporary agent
	// directories can be removed reliably on Windows.
	AgentStorage.close();
