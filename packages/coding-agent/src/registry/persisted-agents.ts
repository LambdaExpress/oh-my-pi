...
export async function registerPersistedSubagents(
	...
	await registerPersistedSubagentsFromDir(registry, root, undefined, scopeId, vibeOwnedIds);
}
...
async function registerPersistedSubagentsFromDir(
	...
	for (const entry of entries) {
		...
		if (isAdvisorTranscriptName(entry.name)) {
			...
			if (existing?.sessionFile !== sessionFile || existing.scopeId !== scopeId) {
		...
		await registerPersistedSubagentsFromDir(registry, path.join(dir, id), id, scopeId, vibeOwnedIds);
	}
}
