...
- Execution realms: direct helpers (`tab.evaluate`, `tab.click`, `tab.observe`, element handles) evaluate in the page's main world and see the app's own `window` globals. Inside `tab.run`, the raw page's `evaluate`, `evaluateHandle`, `$eval`, `$$eval`, `waitForFunction`, and element-handle evaluation default to Puppeteer's isolated (utility) world, which cannot see them. Select the main world per call with a `//!world=main` or `/*!world=main*/` directive as the first comment of the function body or of a string expression — `page.evaluate(() => { /*!world=main*/ return window.appGlobal; })` — or call `page.mainFrame().mainRealm().evaluate(fn)`.
- `page.addScriptTag` runs its script in the main world; read the effect back through a main-world evaluation, never a plain `page.evaluate`.
- Page console output and page errors are not delivered to `page.on("console")`/`page.on("pageerror")` in this build. Capture them inside the page instead (a `page.evaluateOnNewDocument` or `addScriptTag` hook pushing to a `window` array) and read that array in the main world.
