...
{{#if spawns}}agent(prompt, agent?="{{spawnDefaultAgent}}", label?=None, schema?=None, schema{{#ifAny py rb jl}}_mode{{else}}Mode{{/ifAny}}?="permissive", isolated?=None, apply?=None, merge?=None, handle?=False) → str | dict
