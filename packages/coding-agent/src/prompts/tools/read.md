...
- `?q=<question>` — image only (also `.svg:img?q=`, `attachment://N?q=`, `local://…?q=`): vision-model answer as text; unavailable service → local image or metadata with a failure notice
...
- Documents → extracted text. Notebooks → editable cells. Images → decoded inline; `img.png?q=<question>` asks a vision model and normally returns text (spares context; works on any model). Videos → preview grid plus metadata. SVGs read as text unless `:img` is specified. `:raw` bypasses converters.
