...
export interface OutputSinkOptions {
	artifactPath?: string;
	artifactId?: string;
	/** Inline spill threshold (bytes). Default DEFAULT_MAX_BYTES. */
...
export class OutputSink {
	...
	#willOverflow(dataBytes: number): boolean {
		// Trigger artifact mirroring when the next chunk would push the visible
		// head + tail budget over the spill threshold.
	...
	#pushTail(chunk: string, dataBytes: number): void {
		...
		// Overflow: keep only the configured tail window in memory.
		this.#truncated = true;
