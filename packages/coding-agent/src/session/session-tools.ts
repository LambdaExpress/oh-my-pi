...
export class SessionTools {
	...
	async replaceSshCapabilityTool(
		...
		if (this.#xdev && !pinnedTopLevel) {
			const previousMounted = new Set(this.#xdev.mountedNames);
			if (tool) {
				this.#toolRegistry.set(name, tool);
				this.#builtInToolNames.add(name);
				this.#xdev.builtInNames.add(name);
				this.#xdev.mountedNames.add(name);
			} else {
				this.#builtInToolNames.delete(name);
				this.#xdev.builtInNames.delete(name);
				this.#xdev.mountedNames.delete(name);
				this.#toolRegistry.delete(name);
			}
			if (this.getActiveToolNames().includes(name)) {
				const activeTools = this.#host.agent.state.tools.filter(activeTool => activeTool.name !== name);
				this.#host.agent.setTools(activeTools);
				this.#setActiveToolNames?.(activeTools.map(activeTool => activeTool.name));
			}
		...
		const nextEnabled = previousEnabled.filter(toolName => toolName !== name && this.#toolRegistry.has(toolName));
		if (tool && (wasEnabled || (options?.activateIfAvailable && !wasAvailable))) {
			nextEnabled.push(name);
		}
