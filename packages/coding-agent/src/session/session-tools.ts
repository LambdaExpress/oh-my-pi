...
export class SessionTools {
	...
	async replaceSshCapabilityTool(
		...
		if (this.#xdevRegistry && !pinnedTopLevel) {
			...
			this.#mountedXdevToolNames = new Set(previousMounted);
			this.#xdevRegistry.setBuiltin(name, tool);
			this.#toolRegistry.delete(name);
			if (tool) {
				this.#builtInToolNames.add(name);
				this.#mountedXdevToolNames.add(name);
			} else {
				this.#builtInToolNames.delete(name);
				this.#mountedXdevToolNames.delete(name);
			}
		...
		this.#xdevRegistry?.setBuiltin(name, null);
		if (tool) {
