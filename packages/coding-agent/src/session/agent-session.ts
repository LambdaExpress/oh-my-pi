...
export class AgentSession {
	...
	async #queueUserMessage(
		...
		const videoAttachmentNotices = this.#createVideoAttachmentNotices(images, timestamp);
		const preparation: QueuedMessagePreparation | undefined =
			mode === "aside"
				? undefined
				: {
						deliverAs: mode,
		...
		if (mode === "aside") {
			...
			records.push({ role: "user", content, attribution: "user", timestamp });
			this.#irc.queueAside(records);
		...
		const userMessage: AgentMessage =
			mode === "followUp"
				? { role: "user", content, attribution: "user", timestamp }
				: { role: "user", content, steering: true, attribution: "user", timestamp };
	...
	/** Install the interactive title-generation UI hook. */
	setTitleGenerationStart(handler: (() => void) | undefined): void {
		this.#titleGenerationStart = handler;
	}

	/** Restore a typed prompt that was dropped before it reached the agent. */
