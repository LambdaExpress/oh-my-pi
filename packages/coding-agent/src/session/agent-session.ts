...
export interface AgentSessionConfig {
	...
	/** Rebuild the SSH file-transfer tool from current capability discovery results. */
...
export class AgentSession {
	...
	reloadSshTransferTool: (() => Promise<Tool | null>) | undefined;
	...
	async #advancePrewalk(liveMessages: AgentMessage[], context: AgentTurnEndContext | undefined): Promise<void> {
		...
		await this.waitForMessagePersistence(context.message);
		for (const toolResult of context.toolResults) {
			await this.waitForMessagePersistence(toolResult);
		}
	...
	#processAgentEvent = async (rawEvent: AgentEvent): Promise<void> => {
		...
		// A fresh run supersedes the previously settled and pruned refusal turn.
	...
	async refreshSshTools(options?: { activateIfAvailable?: boolean }): Promise<void> {
		...
		if (!this.reloadSshTransferTool) return;
		...
		const previousActiveToolNames = this.getActiveToolNames();
		const wasActive = previousActiveToolNames.includes(name);
		const wasAvailable = this.#toolRegistry.has(name);
		if (refreshedTool) this.#toolRegistry.set(name, refreshedTool);
		else this.#toolRegistry.delete(name);

		const nextActiveToolNames = previousActiveToolNames.filter(
			toolName => toolName !== name && this.#toolRegistry.has(toolName),
		);
		if (refreshedTool && (wasActive || (options?.activateIfAvailable && !wasAvailable))) {
	 ...
	 * and `EditTool`), and the signature reads them live on every call. A settings
	 * flip that mutates rendered metadata therefore changes the signature the next
	 * time `#applyActiveToolsByName` runs. Do not cache per-tool strings without
	 ...
	 * `eagerTasks`, `intentField`, `secretsEnabled`). Closure-captured values cannot
	...
	async #applyAutoThinkingLevel(promptText: string, generation: number): Promise<void> {
		...
		if (this.#magicKeywordEnabled("ultrathink") && containsUltrathink(promptText)) {
			...
			// and jump straight to the highest effort eligible for auto on this
			// model, including Max when the model supports it.
