...
import type { ImageAttachmentEntry, Tool } from "../tools";
...
import { isAdvisorCard } from "@oh-my-pi/pi-tui/chat/advisor-cards";
...
/** Local queue bookkeeping for a staged queued message; distinct from the agent-core
 *  commit protocol (`QueuedMessagePreparation`), which prepares a claimed batch. */
type QueuedMessagePreparationRecord = {
...
export class AgentSession {
	...
	#queuedMessagePreparations: QueuedMessagePreparationRecord[] = [];
	...
	constructor(config: AgentSessionConfig) {
		...
		const prewalkHost: PrewalkCoordinatorHost = {
			...
			waitForSessionMessagePersistence: message => this.waitForMessagePersistence(message),
			localProtocolOptions: () => this.#localProtocolOptions(),
		};
	...
	#queueMessageEndPersistence(message: AgentMessage, persistenceTarget: PersistenceTarget): Promise<void> {
		const key = sessionMessagePersistenceKey(message);
		const pending = this.#messageEndPersistenceTail
			.then(() => this.#persistMessageEnd(message, persistenceTarget))
			.finally(() => {
	...
	#persistMessageEnd(message: AgentMessage, persistenceTarget: PersistenceTarget): void {
		if (!this.#isSamePersistenceTarget(persistenceTarget)) {
	...
	#processAgentEvent = async (rawEvent: AgentEvent): Promise<void> => {
		...
		const messageEndPersistence =
			event.type === "message_end"
				? this.#queueMessageEndPersistence(event.message, eventPersistenceTarget)
				: undefined;
	...
	async #disposeOwnedAsyncJobs(): Promise<void> {
		...
		await this.#cancelOwnAsyncJobs(manager ? ASYNC_JOB_MANAGER_SHUTDOWN_REASON : undefined);
		if (!manager) return;
	...
	async #dispatchPrompt(
		...
		if (this.isStreaming) {
			...
			// Queue the keyword notices in the same ordered reservation as the user
			// message so a later text-only submission cannot overtake image work.
	...
	async followUp(text: string, images?: ImageContent[], options?: FollowUpOptions): Promise<void> {
		...
		this.#enqueuePreparedQueuedMessageBatch("followUp", [
			...(imageDescriptionNotice ? [imageDescriptionNotice] : []),
			{
				...
				timestamp: submittedAt,
				// Marks this as a run-initiating synthetic prompt during transcript replay.
				synthetic: true,
			},
		]);
	}
	...
	async #queueUserMessage(
		...
		const preparation: QueuedMessagePreparationRecord | undefined =
			mode === "aside"
				? undefined
				: {
						deliverAs: mode,
		...
		if (preparation) this.#queuedMessagePreparations.push(preparation);
		let normalizedImages: ImageContent[] | undefined;
		try {
			normalizedImages = preprocessed ? preprocessed.images : await this.#normalizeImagesForModel(images);
		} catch (error) {
			if (preparation) {
				this.#cancelQueuedMessagePreparation(preparation);
				// Mark the failed reservation ready so the ordered drain can remove
				// it and unblock any later preparation already waiting behind it.
				preparation.readyMessages = [];
		...
		if (mode === "aside") {
			...
			const records: AgentMessage[] = [...prependMessages, ...videoAttachmentNotices];
		...
		const queuedPreparation = preparation!;
		...
		queuedPreparation.restorableMessage = userMessage;

		const accepted = Promise.withResolvers<void>();
		const descriptionPromise = preprocessed
			? Promise.resolve(preprocessed.descriptionNotice)
			: normalizedImages?.length
				? this.#buildImageDescriptionNotice(normalizedImages, queuedPreparation.abortController?.signal, () => {
						if (queuedPreparation.cancelled) return;
						// Approval is the user-visible acceptance boundary. The
						// description request continues in the tracked background task.
						queuedPreparation.visible = true;
						queuedPreparation.displayableCount = 1;
						accepted.resolve();
					})
		...
		const completion = descriptionPromise
			...
			.then(imageDescriptionNotice => {
				...
				queuedPreparation.readyMessages = readyMessages;
				queuedPreparation.visible = true;
				queuedPreparation.displayableCount = readyMessages.filter(isDisplayableQueuedMessage).length;
				this.#drainQueuedMessagePreparations();
			})
	...
	#cancelQueuedMessagePreparation(preparation: QueuedMessagePreparationRecord): void {
	...
	clearQueue(options?: { forInterrupt?: boolean }): {
		...
		const keptPreparations: QueuedMessagePreparationRecord[] = [];
	...
	/** Install the interactive title-generation UI hook. The hook may return
	 *  cleanup to run when generation settles. */
