...
/** Per-session connection path; the digest keeps credential revisions isolated. */
export function getControlPath(host: SSHConnectionTarget): string {
	return path.join(CONTROL_DIR, `${getSshConnectionKey(host)}.sock`);
}

/** OpenSSH-native template retained for callers that do not have a resolved host. */
