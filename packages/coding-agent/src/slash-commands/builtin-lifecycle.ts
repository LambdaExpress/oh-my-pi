		...
		handle: async (command, runtime) => {
			...
			const runRename = async (): Promise<void> => {
				...
				try {
					...
					if (!title) {
						await runtime.output(t("Could not generate a session title. Use /rename <title> to set one."));
						return;
					}
					...
					await runtime.notifyTitleChanged?.();
					if (!isCurrent()) return;
					await runtime.output(t("Session renamed to {title}.", { title }));
				} catch (err) {
					if (!isCurrent()) return;
					if (command.args || !runtime.runCommandInBackground) throw err;
					await runtime.output(t("Rename failed: {error}", { error: errorMessage(err) }));
				}
			};
