...
export class BtwController {
	...
	#branchUnavailableReason(): string | undefined {
		...
		if (this.#transitionCount > 0) return t("a session operation is in progress");
		if (!this.#visible || this.#activeRequest?.component.isBranchable() !== true) return t("the answer is not ready");
		// Inline branch promotion carries one pair; do not drop earlier side turns.
		if (this.#activeRequest.history?.length) return t("multi-turn side conversations remain in BTW history");
	...
	async handleBranch(): Promise<boolean> {
		...
		try {
			...
			await this.ctx.handleBtwBranch(
				...
				leafId,
				sessionId,
				this.#lastSkillPreludeMessage ? [this.#lastSkillPreludeMessage] : undefined,
			);
			return true;
		} catch (error) {
	...
	async flush(timeoutMs = 10_000): Promise<void> {
		await withTimeout(
			this.#drainWrites(),
			timeoutMs,
			t("BTW history is still being saved. The session operation was stopped; retry when storage responds."),
		);
	}
			...
			throw new Error(
				sanitizeErrorLine(
					t(
						...
						{ error: sanitizeErrorLine(failure) },
					),
	...
	async withSessionMove(operation: () => Promise<boolean>): Promise<boolean> {
		if (
			...
			this.ctx.showStatus(t("Wait for the current /btw answer to finish or cancel it before moving."), {
				dim: true,
			});
			return false;
		}
	...
	async #loadHistory(): Promise<BtwHistoryStore> {
		...
		if (this.#storeSessionId !== sessionId || this.#storeArtifactsDir !== artifactsDir) {
			...
			if (
				...
				throw new Error(t("The session changed while opening BTW history."));
			}
	...
	/**
	 * Resolve a `/skill:<name> <question>` side question into the question the
	 * panel shows plus the skill context replayed on branch promotion.
	 * Returns undefined when the invocation is unknown or unusable.
	 */
	async #resolveSkill(
		question: string,
	): Promise<{ question: string; skill: BtwRequest["skill"] | undefined } | undefined> {
		const parsedSkill = parseSkillInvocation(question);
		if (!parsedSkill) return { question, skill: undefined };
		...
		try {
			...
			const built = await buildSkillPromptMessage(skill, { args: parsedSkill.args }, "user", { body });
			return {
				question: parsedSkill.args,
				skill: {
					name: skill.name,
			...
			this.ctx.showError(
				t("Failed to load skill: {error}", { error: error instanceof Error ? error.message : String(error) }),
			);
			return undefined;
		}
	}
	...
	async #start(question: string, recordId?: string, signal?: AbortSignal): Promise<boolean> {
		...
		if (this.#starting || this.#branchInFlight || this.#transitionCount > 0) {
			this.ctx.showStatus(t("A /btw action is in progress. Please wait."), { dim: true });
			return false;
		}
		if (
			trimmedQuestion &&
			...
			this.ctx.showStatus(t("A /btw question is still running. Open /btw to view it or cancel it first."), {
				dim: true,
			});
			return false;
		}
		...
		this.#starting = true;
		try {
			const resolved = await this.#resolveSkill(trimmedQuestion);
			if (!resolved) return false;
			trimmedQuestion = resolved.question;
			const requestSkill = resolved.skill;
			...
			if (recordId && (!previous || getBtwLatestTurn(previous).status === "running")) {
				this.ctx.showStatus(t("This side conversation is unavailable or still running."), { dim: true });
				return false;
			}
	...
	async #runRequest(request: BtwRequest): Promise<void> {
		try {
			...
			if (!model) throw new Error(t("No active model available for /btw."));
