...
export class EventController {
	...
	completePendingLocalExecution(component: Component): void {
		...
		const continuationTimeline = splitAssistantMessageToolTimeline(
	 ...
	 * preserved component. Focus attach clears {@link #backgroundToolCallIds}
	...
	markBackgroundTaskCalls(toolCallIds: Iterable<string>): void {
		for (const toolCallId of toolCallIds) {
			if (this.ctx.pendingTools.has(toolCallId)) this.#backgroundToolCallIds.add(toolCallId);
		}
	}
			...
			if (terminalFailurePinned) {
				const terminalError = this.#restorePinnedErrorInline
					? t("Retry failed after {attempt} attempts: {error}", {
							...
							error: event.finalError || pinnedError || t("Unknown error"),
						})
					: (pinnedError ?? event.finalError);
