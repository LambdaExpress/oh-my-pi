...
export class CommandController {
	...
	async handleDumpCommand(): Promise<void> {
		try {
			...
			try {
				...
				sidecarError = error instanceof Error ? error.message : t("Unknown error");
			}
			...
			if (sidecarPath) statusParts.push(t("LLM request JSON: {path}", { path: sidecarPath }));
			if (sidecarError) statusParts.push(t("LLM request JSON unavailable: {error}", { error: sidecarError }));
			this.ctx.showStatus(statusParts.join("\n"));
		} catch (error: unknown) {
			this.ctx.showError(
				t("Failed to copy session: {error}", {
					error: error instanceof Error ? error.message : t("Unknown error"),
				}),
			);
		}
	...
	handleAdvisorDumpCommand(isRaw = false) {
		try {
			...
			this.ctx.showError(
				t("Failed to copy advisor history: {error}", {
					error: error instanceof Error ? error.message : t("Unknown error"),
				}),
			);
		}
	...
	async handleDebugTranscriptCommand(): Promise<void> {
		try {
			...
			this.ctx.showError(
				t("Failed to write debug transcript: {error}", {
					error: error instanceof Error ? error.message : t("Unknown error"),
				}),
			);
		}
	...
	async handleShareCommand(): Promise<void> {
		...
		if (customShare) {
			...
			try {
				...
				if (typeof result === "string") {
					this.ctx.showStatus(t("Share URL: {url}", { url: result }));
					this.openInBrowser(result);
				} else if (result) {
					const parts: string[] = [];
					if (result.url) parts.push(t("Share URL: {url}", { url: result.url }));
		...
		try {
			...
			if (!loader.signal.aborted) {
				restoreEditor();
				this.ctx.showError(
					t("Failed to share session: {error}", {
						error: error instanceof Error ? error.message : t("Unknown error"),
					}),
				);
			}
	...
	async handleSessionCommand(): Promise<void> {
		...
		// Append-only context
		{
			...
			info += `${theme.fg("dim", t("Append-Only:"))} ${activeLabel} (${t("setting:")} ${settingLabel})\n`;
		}
		...
		if (this.ctx.mcpManager) {
			...
			if (mcpServers.length === 0) {
				...
				for (const name of mcpServers) {
					...
					info += `${theme.fg("dim", `${name}:`)} ${theme.fg("success", t("connected"))} ${theme.fg("dim", `(${t("{count} tools", { count: toolCount })})`)}\n`;
				}
			}
		}
		...
		if (stats.advisors.length > 1 || (stats.configured && !stats.active)) {
			...
			for (const a of stats.advisors) {
				...
				if (a.status === "running" || a.status === "quota_exhausted") {
					...
					info += `${theme.fg("dim", t("Spend:"))} ${a.tokens.input.toLocaleString()} ${t("in")} / ${a.tokens.output.toLocaleString()} ${t("out")}`;
	...
	async handleUsageCommand(reports?: UsageReport[] | null): Promise<void> {
		...
		if (!usageReports) {
			...
			try {
				...
				this.ctx.showError(
					t("Failed to fetch usage data: {error}", {
	...
	async handleMemoryCommand(text: string): Promise<void> {
		...
		if (action === "stats" || action === "diagnose") {
			...
			try {
				...
				showMarkdownPanel(
					this.ctx,
					t("Memory {section}", { section: action === "stats" ? t("Stats") : t("Diagnostics") }),
					payload,
				);
			} catch (error) {
				this.ctx.showError(
					t("Memory {action} failed: {error}", {
						action,
						error: error instanceof Error ? error.message : String(error),
					}),
				);
	...
	async #mmList(state: HindsightSessionState): Promise<void> {
		...
		try {
			...
			if (items.length === 0) {
				this.ctx.showStatus(t("No mental models on bank {bank}.", { bank: state.bankId }));
				return;
			}
			const lines = items
				.slice()
	...
	async #mmRefresh(state: HindsightSessionState, id: string | undefined): Promise<void> {
		try {
			if (id) {
				...
				this.ctx.showStatus(t("Refresh queued for mental model {id}.", { id }));
			} else {
	...
	async #mmHistory(state: HindsightSessionState, id: string): Promise<void> {
		try {
			...
			if (history.length === 0) {
				this.ctx.showStatus(t("No history recorded for {id}.", { id }));
				return;
			}
	...
	async #mmSeed(state: HindsightSessionState): Promise<void> {
		try {
			...
			if (seeds.length === 0) {
				this.ctx.showStatus(t("No built-in seeds apply to scoping={scoping}.", { scoping: config.scoping }));
				return;
			}
	...
	async handleFreshCommand(): Promise<void> {
		...
		this.ctx.ui.requestRender();
		this.ctx.showStatus(
			t("Fresh provider session started ({count} {state} pruned).", {
	...
	async handleResetContextCommand(): Promise<void> {
		...
		this.ctx.statusLine.invalidate();
		this.ctx.updateEditorBorderColor();
		const noun = result.droppedCount === 1 ? t("message") : t("messages");
		this.ctx.present([
			new Spacer(1),
			new Text(
				`${theme.fg(
					"accent",
					`${theme.status.success} ${t("Context reset — {count} {noun} dropped; session continues.", {
	...
	async handleMoveCommand(targetPath?: string): Promise<void> {
		...
		if (!isDirectory) {
			...
			if (!parentExists) {
				this.ctx.showError(
					t('Cannot create "{name}": parent directory does not exist', {
	...
	async executeCompaction(
		...
		try {
			...
			if (error instanceof CompactionCancelledError) {
				...
				this.ctx.showError(t("Compaction failed: {error}", { error: message }));
			}
		} finally {
			compactingLoader.stop();
			this.ctx.statusContainer.disposeChildren();
	...
	async handleHandoffCommand(customInstructions?: string): Promise<void> {
		...
		try {
			...
			if (result.savedPath) {
				this.ctx.showStatus(t("Handoff document saved to: {path}", { path: result.savedPath }));
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
...
function formatAggregateAmount(limits: UsageLimit[]): string {
	...
	if (fractions.length === limits.length && fractions.length > 0) {
		...
		return t("{pct}% free", { pct: formatNumber(avgRemaining) });
	}
	...
	if (amounts.length === limits.length && amounts.length > 0) {
		...
		return t("{pct}% free", { pct: formatNumber(remainingPct) });
	}
	...
	if (uniqueAccountIds.size > 0) {
		return `${uniqueAccountIds.size} ${uniqueAccountIds.size === 1 ? t("acct") : t("accts")}`;
	}
...
function resolveResetRange(limits: UsageLimit[], nowMs: number): string | null {
	...
	const labels = new Set(windows.map(window => window.resetLabel ?? t("resets")));
	const verb = labels.size === 1 ? [...labels][0]! : t("resets");
...
function renderUsageBar(limit: UsageLimit, uiTheme: typeof theme, barWidth: number): string {
	...
	if (usedAmount !== undefined && isUsedOnlyAbsoluteAmount(limit)) {
		...
		return uiTheme.fg("dim", truncateJobLabel(t("{used} used", { used }), barWidth));
	}
	const fraction = resolveUsedFraction(limit);
