...
import { formatDuration, normalizePathForComparison, Snowflake, sanitizeText } from "@oh-my-pi/pi-utils";
...
import { parseSubcommand } from "../../slash-commands/helpers/parse";
...
import {
	...
	type WorktreeSelectorResult,
} from "../components/worktree-selector";
...
async function existingFile(filePath: string | null | undefined): Promise<string | null> {
...
async function ensureDirectory(filePath: string): Promise<boolean> {
	try {
...
function selectorItemFromRecord(record: ManagedWorktreeRecord): WorktreeSelectorItem {
	return {
		id: record.id,
		name: record.name,
		state: record.state,
...
export class CommandController {
	...
	async handleWorktreeCommand(actionText?: string): Promise<void> {
		const { verb, rest } = parseSubcommand(actionText ?? "");
		if (!verb) {
			await this.#showWorktreeBrowser();
			return;
		}
		...
		try {
			switch (verb) {
				...
				case "merge":
					...
					return;
				case "remove":
					await this.#removeWorktree(rest, false);
					return;
				case "branch":
					await this.#branchWorktreeFromArgs(rest);
					return;
				case "path":
					await this.#showWorktreePath(rest);
					return;
				case "restore":
					await this.#restoreWorktree(rest);
					return;
				case "prune":
					this.ctx.showWarning("Use `omp worktree prune` to clean up managed worktrees.");
					return;
				default:
					this.ctx.showError(WORKTREE_USAGE);
			}
		} catch (err) {
			this.ctx.showError(`worktree ${verb} failed: ${err instanceof Error ? err.message : String(err)}`);
		}
	}

	async #managedRecords(): Promise<ManagedWorktreeRecord[]> {
		const items = await listManagedWorktrees(this.ctx.sessionManager.getCwd());
	...
	async #showWorktreeList(): Promise<void> {
		const records = await this.#managedRecords();
		if (records.length === 0) {
			this.ctx.showStatus("No managed worktrees.");
			return;
		}
		const lines = ["Managed worktrees:"];
		for (const record of records) {
			const location =
				record.state === "snapshotted" && record.snapshotPath ? record.snapshotPath : targetCwdForRecord(record);
			const ref = record.branch ?? record.baseRef;
			const session = record.title ? ` · ${record.title}` : "";
			lines.push(`- ${record.name} (${record.state}, ${ref}) ${shortenPath(location)}${session}`);
		}
		this.ctx.showStatus(lines.join("\n"), { dim: false });
	}

	async #showWorktreeBrowser(): Promise<void> {
		const records = await this.#managedRecords();
		const items = records.map(selectorItemFromRecord);
		const result = await this.ctx.showHookCustom<WorktreeSelectorResult | undefined>(
			(_tui, _theme, _keybindings, done) =>
				new WorktreeSelectorComponent(items, {
					onPick: done,
		...
		if (!result) return;
		await this.#executeWorktreeSelectorAction(result);
	}

	async #executeWorktreeSelectorAction(result: WorktreeSelectorResult): Promise<void> {
		try {
			switch (result.action) {
				case "switch":
					await this.#switchWorktree(result.id);
					return;
				case "move-current":
					await this.#moveCurrentSessionToWorktree(result.id);
					return;
				case "merge":
					await this.#mergeWorktree(result.id);
					return;
				case "branch": {
					const branch = await this.ctx.showHookInput("Create branch", "Branch name");
					if (!branch?.trim()) return;
					await this.#branchWorktree(result.id, branch.trim());
					return;
				}
				case "remove": {
					await this.#removeWorktree(result.id, true);
					return;
				}
				case "restore":
					await this.#restoreWorktree(result.id);
					return;
			}
		} catch (err) {
			this.ctx.showError(`Managed worktree action failed: ${err instanceof Error ? err.message : String(err)}`);
		}
	}

	async #managedRecord(idOrName: string): Promise<ManagedWorktreeRecord | null> {
		...
		if (!record) this.ctx.showError(`Unknown managed worktree: ${key}`);
		return record;
	}

	async #switchWorktree(idOrName: string): Promise<void> {
		if (this.ctx.session.isStreaming) {
			this.ctx.showWarning("Wait for the current response to finish or abort it before moving.");
			return;
		}
		...
		if (!record) return;
		if (!(await ensureDirectory(record.worktreeRoot))) {
			this.ctx.showError("Managed worktree directory is missing; remove metadata or restore from snapshot.");
			return;
		}

		const targetCwd = targetCwdForRecord(record);
		const sessionFile = await existingFile(record.sessionFile);
		this.ctx.clearTransientSessionUi();
		if (sessionFile) {
			const previousCwd = this.ctx.sessionManager.getCwd();
			const switched = await this.ctx.session.switchSession(sessionFile);
			if (!switched) {
				this.ctx.showStatus("Switch cancelled");
				return;
			}
			const newCwd = this.ctx.sessionManager.getCwd();
			if (normalizePathForComparison(newCwd) !== normalizePathForComparison(previousCwd)) {
			...
			await this.#refreshAfterWorktreeSessionChange(
				`Switched to managed worktree ${record.name}: ${shortenPath(newCwd)}`,
			);
			return;
		}

		const created = await this.ctx.session.newSession();
		if (!created) {
			this.ctx.showStatus("Switch cancelled");
			return;
		}
		this.ctx.resetObserverRegistry();
		await this.ctx.sessionManager.moveTo(targetCwd);
		await this.ctx.applyCwdChange(targetCwd);
		await this.#writeCurrentSessionToRecord(record);
		await this.#refreshAfterWorktreeSessionChange(
	...
	async #moveCurrentSessionToWorktree(idOrName: string): Promise<void> {
		if (this.ctx.session.isStreaming) {
			this.ctx.showWarning("Wait for the current response to finish or abort it before moving.");
			return;
		}
		...
		this.ctx.present([
			new Spacer(1),
			new Text(
				`${theme.fg("accent", `${theme.status.success} Moved current session to managed worktree ${record.name}`)}`,
				1,
				1,
			),
		]);
	...
	async #writeCurrentSessionToRecord(record: ManagedWorktreeRecord): Promise<void> {
		const now = new Date().toISOString();
		await writeManagedWorktreeRecord({
			...record,
			sessionFile: this.ctx.sessionManager.getSessionFile() ?? null,
			sessionId: this.ctx.sessionManager.getSessionId(),
			title: this.ctx.sessionManager.getSessionName() ?? record.title,
			updatedAt: now,
			lastUsedAt: now,
		});
	}

	async #refreshAfterWorktreeSessionChange(message: string): Promise<void> {
		setSessionTerminalTitle(this.ctx.sessionManager.getSessionName(), this.ctx.sessionManager.getCwd());
		this.ctx.statusLine.invalidate();
		this.ctx.statusLine.resetActiveTime();
		this.ctx.updateEditorBorderColor();
	...
	async #mergeWorktree(idOrName: string): Promise<void> {
		const record = await this.#managedRecord(idOrName);
		if (!record) return;
		const updated = await mergeManagedWorktree({ cwd: this.ctx.sessionManager.getCwd(), idOrName: record.id });
		this.ctx.showStatus(`Applied managed worktree ${updated.name} to local checkout.`);
	}

	async #removeWorktree(idOrName: string, confirm: boolean): Promise<void> {
		const record = await this.#managedRecord(idOrName);
		if (!record) return;
		if (confirm) {
			const accepted = await this.ctx.showHookConfirm("Remove managed worktree?", record.name);
			if (!accepted) return;
		}
		const removed = await removeManagedWorktree({ cwd: this.ctx.sessionManager.getCwd(), idOrName: record.id });
		if (removed) {
			this.ctx.showStatus(`Removed managed worktree ${removed.name}.`);
		} else {
			this.ctx.showStatus(`Managed worktree ${record.name} is already gone.`);
		}
	}

	async #branchWorktreeFromArgs(rest: string): Promise<void> {
		const parsed = parseSubcommand(rest);
		if (!parsed.verb || !parsed.rest) {
			this.ctx.showError("Usage: /worktree branch <id|name> <branch>");
			return;
		}
		await this.#branchWorktree(parsed.verb, parsed.rest);
	}

	async #branchWorktree(idOrName: string, branch: string): Promise<void> {
		const record = await this.#managedRecord(idOrName);
		if (!record) return;
		...
		this.ctx.showStatus(`Managed worktree ${updated.name} created branch ${updated.branch ?? branch}.`);
	}

	async #showWorktreePath(idOrName: string): Promise<void> {
		const record = await this.#managedRecord(idOrName);
		if (!record) return;
		this.ctx.showStatus(targetCwdForRecord(record), { dim: false });
	}

	async #restoreWorktree(idOrName: string): Promise<void> {
		const record = await this.#managedRecord(idOrName);
		if (!record) return;
		const result = await restoreManagedWorktree({ cwd: this.ctx.sessionManager.getCwd(), idOrName: record.id });
		this.ctx.showStatus(`Restored managed worktree ${result.record.name}: ${shortenPath(result.targetCwd)}`);
	}
