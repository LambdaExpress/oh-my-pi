...
export class SessionFocusController {
	...
	async #detachToMain(): Promise<void> {
		...
		if (attached && this.#focusedAgentId === undefined) this.ctx.showStatus(t("Returned to main session"));
	}
	...
	async #attach(target: AgentSession): Promise<boolean> {
		...
		try {
			...
			if (generation !== this.#attachGeneration) return false;
			await this.ctx.renderInitialMessages({ clearTerminalHistory: true, recoverCompletedRunAnchor: true });
			if (generation !== this.#attachGeneration) return false;
