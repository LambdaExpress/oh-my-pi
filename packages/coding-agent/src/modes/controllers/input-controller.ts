...
export class InputController {
	...
	setupKeyHandlers(): void {
		...
		for (const key of this.ctx.keybindings.getKeys("app.jobs.hub")) {
			this.ctx.editor.setCustomKeyHandler(key, () => this.ctx.showBackgroundJobsHub());
		}
		...
		this.#enhancedPaste = new EnhancedPasteController({
			...
			pasteImage: async image => {
				...
				if (focused && focused !== this.ctx.editor && hasPasteText(focused)) {
					this.ctx.showStatus(t("Image paste is not supported in this prompt"));
					return;
				}
				await this.#normalizeAndInsertPastedImage(
					image,
					t("Unsupported pasted image format: {mime}", { mime: image.mimeType }),
				);
			},
		...
		this.ctx.editor.onSubmit = async (text: string) => {
			...
			if (text) {
				...
				const slashResult = await executeBuiltinSlashCommand(text, {
					...
					input,
					draftDetached,
					draftText: draftDetached ? submittedDraftText : undefined,
				});
				if (slashResult === true) {
			...
			if (this.ctx.collabGuest) {
				if (text.startsWith("/")) {
					this.ctx.showStatus(
						t("{command} is host-only during a collab session", {
				...
				if (text.startsWith("!") || parsePythonCommandInput(text)) {
					this.ctx.showStatus(t("Local execution is host-only during a collab session"));
					this.ctx.editor.setText("");
					return;
				}
				if (this.ctx.collabGuest.readOnly) {
					// Keep the typed text: the prompt was not consumed.
					this.ctx.showStatus(t("This collab link is read-only — prompting is disabled"));
					return;
				}
			...
			if (text.startsWith("!")) {
				...
				if (command) {
					if (this.ctx.session.isBashRunning) {
						this.ctx.showWarning(t("A bash command is already running. Press Esc to cancel it first."));
	...
	async #submitToFocusedSession(text: string, streamingBehavior: "steer" | "followUp"): Promise<void> {
		...
		if (text && (text.startsWith("/") || text.startsWith("!") || parsePythonCommandInput(text))) {
			this.ctx.showStatus(t("Commands run in the main session — press ←← to return first"));
	...
	handleCtrlZ(): void {
		...
		if (process.platform === "win32") {
			this.ctx.showStatus(t("Suspend is not supported on this platform"));
			return;
		}
		...
		try {
			...
			this.ctx.showError(t("Failed to suspend: {reason}", { reason }));
		}
	}
	...
	handleDequeue(): void {
		...
		if (!popped) {
			this.ctx.showStatus(t("No queued messages to restore"));
			return;
		}
		...
		this.ctx.showStatus(t("Restored last queued message to editor"));
	}
	...
	async handleRetry(): Promise<void> {
		if (this.ctx.collabGuest) {
			this.ctx.showStatus(t("/retry is host-only during a collab session"));
			return;
		}
		...
		if (didRetry) {
			...
			this.ctx.showStatus(t("Nothing to retry"));
		}
	}
	...
	async #queueForYield(
		...
		if (splitMessages.length === 0 && !options.images?.length) {
			this.ctx.editor.clearDraft();
			this.ctx.showWarning(t("Usage: /queue <message> (or start a prompt with -> / =>)"));
			return;
		}
		...
		if (this.ctx.session.isCompacting) {
			...
			this.ctx.showStatus(
				messages.length === 1
					...
					: t("Queued {count} messages for after compaction", { count: messages.length }),
			);
			this.ctx.ui.requestRender();
			return;
		}
		...
		if (queuedCount === messages.length) {
			this.ctx.showStatus(
				startImmediately
					? queuedCount === 1
						? t("Sent queued message")
						: t("Sent first message; queued {count} for later yields", {
								count: queuedCount - 1,
							})
					: queuedCount === 1
						? t("Queued message for when the agent yields")
						: t("Queued {count} messages for when the agent yields", { count: queuedCount }),
			);
		}
		this.ctx.ui.requestRender();
	}
	...
	async #tryPasteClipboardImage(): Promise<boolean> {
		...
		try {
			...
			await this.#normalizeAndInsertPastedImage(
				...
				t("Unsupported clipboard image format: {mime}", { mime: image.mimeType }),
			);
			return true;
		} catch {
			return false;
	...
	async handleImagePathPaste(path: string): Promise<void> {
		try {
			...
			if (!image) {
				...
				this.ctx.ui.requestRender();
				this.ctx.showStatus(t("Pasted path is not a supported image"));
				return;
			}
			await this.#normalizeAndInsertPastedImage(
			...
			this.ctx.editor.pasteText(path);
			this.ctx.ui.requestRender();
			this.ctx.showStatus(t("Failed to read pasted image path"));
		}
	}
	...
	async handleImagePaste(): Promise<boolean> {
		...
		try {
			...
			this.ctx.showStatus(t("Failed to read clipboard"));
			return false;
		} finally {
			finishPaste?.(undefined);
		}
	...
	async presentLargePasteMenu(text: string, lineCount: number): Promise<void> {
		...
		try {
			choice = await this.ctx.showHookSelector(
				t("Pasted {count} lines", { count: lineCount }),
				[
					{
						label: WRAPPED_BLOCK,
						description: t("Wrap the text in <attachment> tags, collapsed to a marker"),
					},
					{ label: LOCAL_FILE, description: t("Save the text to a local://paste file") },
					{ label: INLINE, description: t("Collapse the text to an inline paste marker") },
				],
				{ helpText: t("Esc to paste inline") },
			);
		} catch (error) {
	...
	async #attachPasteAsFile(text: string, lineCount: number): Promise<void> {
		try {
			...
			this.ctx.showStatus(t("Saved {count} pasted lines to local://{name}", { count: lineCount, name }));
		} catch (error) {
		...
		return createPromptActionAutocompleteProvider({
			...
			internalUrlCaller: () => {
				...
				return {
					...
					sshHosts: () => this.ctx.session.getSessionSshHosts(),
				};
			},
	...
	handleCopyCurrentLine(): void {
		...
		if (!text) {
			this.ctx.showStatus(t("Nothing to copy"));
			return;
		}
		try {
			...
			this.ctx.showStatus(t("Copied line: {preview}", { preview }));
		} catch {
			this.ctx.showWarning(t("Failed to copy to clipboard"));
		}
	}
	...
	handleCopyPrompt(): void {
		...
		try {
			...
			const sanitized = sanitizeText(text);
			const preview = sanitized.length > 30 ? `${sanitized.slice(0, 30)}...` : sanitized;
			this.ctx.showStatus(t("Copied: {preview}", { preview }));
		} catch {
			this.ctx.showWarning(t("Failed to copy to clipboard"));
		}
	}
	...
	cycleThinkingLevel(): void {
		if (this.ctx.focusedAgentId) {
			this.ctx.showStatus(t("Model/thinking apply to the main session — press ←← to return first"));
			return;
		}
		const newLevel = this.ctx.session.cycleThinkingLevel();
		if (newLevel === undefined) {
			this.ctx.showStatus(t("Current model does not support thinking"));
		} else {
	...
	async cycleRoleModel(direction: "forward" | "backward" = "forward"): Promise<void> {
		...
		try {
			...
			if (!result) {
				this.ctx.showStatus(t("Only one role model available"));
				return;
			}
	...
	toggleThinkingBlockVisibility(): void {
		...
		if (thinkingOff && !this.ctx.hasDisplayableThinkingContent) {
			this.ctx.showStatus(t("Thinking is off — enable thinking to show blocks"));
			return;
		}
		...
		this.ctx.showStatus(
			t("Thinking blocks: {state}", { state: this.ctx.hideThinkingBlock ? t("hidden") : t("visible") }),
		);
	}
	...
	async openExternalEditor(): Promise<void> {
		...
		if (!editorCmd) {
			this.ctx.showWarning(t("No editor configured. Set $VISUAL or $EDITOR environment variable."));
			return;
		}
