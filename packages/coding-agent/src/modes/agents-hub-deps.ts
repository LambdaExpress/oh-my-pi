...
export function createAgentsHubDeps(
	...
	return {
		...
		generateAgent: async (description, onText) => {
			...
			if (!selectedModel) throw new Error(t("No available model to generate agent specification."));
			...
			try {
				...
				if (!raw) throw new Error(t("No response returned by agent creation architect."));
				return raw;
			} finally {
				unsubscribe();
		...
		saveAgent: async (scope, spec) => {
			...
			if (!targetDir) throw new Error(t("Cannot resolve {scope} agents directory.", { scope }));
