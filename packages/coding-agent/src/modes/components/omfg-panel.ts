...
export class OmfgPanelComponent extends OverlayPanel {
	...
	#footerLine(): string {
		switch (this.#state) {
			...
			case "saved":
				return theme.fg(
					"success",
					`${theme.status.success} ${t("Registered live · {path} · Esc dismiss", { path: replaceTabs(this.#savedPath ?? "saved") })}`,
				);
			case "rejected":
	...
	#contentComponent(): Component {
		...
		if (!text) {
			return new Text(theme.fg("dim", `${theme.status.pending} ${t("Waiting for candidate rule…")}`), 0, 0);
		}
