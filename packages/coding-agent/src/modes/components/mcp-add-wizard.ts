...
export class MCPAddWizard extends Container {
	...
	#renderScopeStep(): void {
		...
		const options = [
			{ value: "user" as const, label: t("User level ({path})", { path: userPathLabel }) },
			{ value: "project" as const, label: t("Project level ({path})", { path: projectPathLabel }) },
		];
	...
	#renderConfirmStep(): void {
		...
		if (this.#state.transport === "stdio") {
			this.#contentContainer.addChild(new Text(t("Command: {command}", { command: this.#state.command }), 0, 0));
			if (this.#state.args) {
				this.#contentContainer.addChild(new Text(t("Args: {args}", { args: this.#state.args }), 0, 0));
			}
		} else {
			this.#contentContainer.addChild(new Text(t("URL: {url}", { url: sanitize(this.#state.url) }), 0, 0));
		}
		...
		if (this.#state.authMethod === "none") {
			...
			if (this.#state.authLocation === "env") {
				this.#contentContainer.addChild(
					new Text(t("Auth: API key via env ({name})", { name: this.#state.envVarName }), 0, 0),
				);
			} else {
				this.#contentContainer.addChild(
					new Text(t("Auth: API key via header ({name})", { name: this.#state.headerName }), 0, 0),
				);
			}
		}

		const scopeLabel = this.#state.scope === "user" ? t("User level") : t("Project level");
		this.#contentContainer.addChild(new Text(t("Scope: {scope}", { scope: scopeLabel }), 0, 0));
