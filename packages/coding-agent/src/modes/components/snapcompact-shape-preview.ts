...
export class SnapcompactShapePreview implements Component {
	...
	render(width: number): readonly string[] {
		...
		const stats = t("full frame {cols}×{rows} cells ≈ {chars} chars ≈ {tokens} tokens", {
			cols: geo.cols,
			rows: geo.rows,
			chars,
			tokens,
		...
		if (entry.state === "rendering") {
			lines.push(theme.fg("dim", `  ${t("rendering sample…")}`));
			return lines;
		}
		if (entry.state === "failed") {
			lines.push(theme.fg("dim", `  ${t("(sample render failed)")}`));
			return lines;
		}
		...
		if (!result?.lines) {
			lines.push(theme.fg("dim", `  ${t("(graphic sample needs Kitty unicode-placeholder graphics)")}`));
			return lines;
		}
		if (result.transmit) {
