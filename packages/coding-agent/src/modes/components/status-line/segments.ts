...
function pickUsageRemainingColor(percent: number): ThemeColor {
...
const contextPctSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		const colorAnsi = colorToAnsi(color, theme.getColorMode());
		...
		if (ctx.autoCompactEnabled && theme.icon.auto) {
			...
			autoIcon = ` ${
				speculation === "running"
					...
					: speculation === "armed"
						? accentIcon
						: `${colorAnsi}${theme.icon.auto}\x1b[39m`
			}`;
		}
		const text = `${colorAnsi}${
