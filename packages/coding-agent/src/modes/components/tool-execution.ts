...
export class ToolExecutionComponent extends Container implements NativeScrollbackLiveRegion {
	...
	updateResult(
		...
		this.#updateDisplay();
		this.#repaintDisplayForResultTopologyChange(
	...
	#displayKey(): string {
		...
		return `${this.#resultVersion}|${this.#expanded}|${this.#isPartial}|${spinnerFrame}|${this.#showImages}|${getThemeEpoch()}|${this.#displayInputVersion}|${this.#backgroundTaskFrozenStyled}|${imageProtocol}|${imageSize}`;
	}
	...
	#rendererFlag(name: "forceFirstResultViewportReset" | "forceResultViewportRepaintOnSettle"): boolean {
	 ...
	 * streamed custom-device placeholder or a collapsed write tail window,
	 * both of which the first result render re-anchors instead of preserving.
	 * Kept as a per-paint fact so a topology-changing update that lands before
	 * the pending rows reach the terminal skips the replay.
	 */
	...
	#repaintDisplayForResultTopologyChange(
		firstResultAfterRepaintShapePaint: boolean,
		partialResultPaintedBeforeSettle: boolean,
		isPartial: boolean,
	): void {
		if (firstResultAfterRepaintShapePaint && this.#rendererFlag("forceFirstResultViewportReset")) {
