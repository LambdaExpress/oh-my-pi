...
import { getAgentDir, getModelDbPath, getProjectDir, setProjectDir } from "@oh-my-pi/pi-utils/dirs";
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	if (settings.get("secrets.enabled")) {
		...
		const allEntries = [...envEntries, ...fileEntries, ...sshPasswordEntries, ...builtinCredentialSecretEntries()];
	...
	try {
		...
		const reloadSshTransferTool = async (): Promise<Tool | null> => {
			...
			const transferTool = await loadSshTransferTool(currentToolSession);
			if (!transferTool) return null;
			return new ExtensionToolWrapper(wrapToolWithMetaNotice(transferTool), extensionRunner) as Tool;
		};
		...
		let currentModel = model; // calibrated per request in transformProviderContext
		...
		const transformContext = async (messages: AgentMessage[], _signal?: AbortSignal) => {
			if (
				...
				classifyModel(currentModel.provider, currentModel.id, { lenient: true }).class === "deepseek"
			) {
		...
		const transformProviderContext = async (context: Context, transformModel: Model): Promise<Context> => {
			...
			// their tool-schema prefix cache (#7404). Apply it before the optional
			// first-turn anchor so the anchor remains the final system/tool payload.
			...
			currentModel = transformModel;
			if (
				anchorPendingForRequest &&
				context.messages.some(m => m.role === "user") &&
				classifyModel(transformModel.provider, transformModel.id, { lenient: true }).class === "deepseek"
			) {
		...
		// budget to the dsh baseline. Consumed at the first stream call so retries
		// reusing a prepared call never re-apply it.
		const anchorAwareStreamFn: StreamFn = (streamModel, context, streamOptions) => {
			const anchored = anchorRequestActive;
			anchorRequestActive = false;
			const optionsForRequest = anchored ? { ...streamOptions, maxTokens: 256000 } : streamOptions;
			return settingsAwareStreamFn(streamModel, context, optionsForRequest);
		};
		...
		agent = new Agent({
			...
			interruptMode: settings.get("steeringSkipPendingOperations") ? "immediate" : "wait",
		...
		session = new AgentSession({
			...
			disconnectOwnedMcpManager: ownedMcpManager
				? (() => {
						...
						return async () => {
							try {
								await ownedMcp.disconnectAll();
							} finally {
								if (MCPManager.instance() === ownedMcp) MCPManager.setInstance(undefined);
							}
						};
					})()
				: undefined,
		...
		const launchDeferredMCPDiscovery = startDeferredMCPDiscovery
			? (() => {
					let started = false;
					return () => {
						if (started) return;
