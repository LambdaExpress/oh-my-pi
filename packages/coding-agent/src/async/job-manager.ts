...
/** Fields retained for session-wide job history after live-job eviction. */
...
export class AsyncJobManager {
	...
	#retainSessionHistory(job: AsyncJob): void {
		const {
			...
			promise: _promise,
			latestDetails: _latestDetails,
			retainedArtifactsCleanup: _retainedArtifactsCleanup,
			...record
		} = job;
		...
		for (const stale of bucket.slice(0, -DEFAULT_SESSION_HISTORY_LIMIT)) {
			this.#sessionHistory.delete(stale.id);
			this.#consumedJobResults.delete(stale.id);
		}
	}
	...
	#deliverDelivery(delivery: AsyncJobDelivery): Promise<void> {
		...
		const promise = (async () => {
			...
			try {
				...
				if (!this.#isDeliverySuppressed(delivery) && this.#jobs.has(delivery.jobId)) {
