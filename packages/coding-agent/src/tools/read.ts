...
import { MAX_IMAGE_INPUT_BYTES, webpExclusionForModel } from "@oh-my-pi/pi-tui/chat/image-loading";
...
function recordFullHashlineContext(
	...
	const tag = snapshot
		? store.recordSnapshotForKey(snapshot.key, fullText)
		: absolutePath && path.isAbsolute(absolutePath)
			? store.recordSnapshot(absolutePath, fullText)
			: undefined;
	if (!tag) return undefined;
	return {
		header: formatReadHashlineHeader(snapshot?.displayPath ?? displayPath, tag),
		tag,
...
function recordInMemorySeenLines(
	...
	if (!seenLines || seenLines.length === 0) return;
	const lines = [...seenLines];
	if (snapshot) {
		getEditStore(session).recordSnapshotForKey(snapshot.key, fullText, lines);
	} else if (absolutePath && path.isAbsolute(absolutePath)) {
		getEditStore(session).recordSnapshot(absolutePath, fullText, lines);
	}
}
...
export function splitImageQuestionTarget(readPath: string): { path: string; question?: string } {
	const supportsQuestion =
		!readPath.includes("://") ||
		readPath.startsWith("attachment://") ||
		readPath.startsWith("file://") ||
		readPath.startsWith("local://");
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	#buildInMemoryTextResult(
		...
		if (hashContext?.tag && hashContext.snapshotKey && seenLines) {
			getEditStore(this.session).recordSeenLinesForKey(hashContext.snapshotKey, hashContext.tag, seenLines);
		}
		if (options.raw === true && options.immutable !== true && rawSeenLines) {
			recordInMemorySeenLines(this.session, options.sourcePath, text, rawSeenLines, options.snapshot);
		}
	...
	#buildInMemoryMultiRangeResult(
		...
		if (options.raw === true && options.immutable !== true && visibleSpans.length > 0) {
		...
		if (!rawPathIsLiteral) {
			...
			if (archivePath) {
				...
				return readArchive(
					...
					signal,
					{ question, completeImageRequest: this.completeImageRequest },
				);
			}
	...
	async #handleInternalUrl(
		...
		const resolveContext: ResolveContext & Pick<WriteContext, "sshHosts"> = {
			cwd: this.session.cwd,
			settings: this.session.settings,
			signal,
		...
		const resource = await internalRouter.resolve(url, resolveContext);
		...
		if (!snapshot) return buildInMemorySelectorResult(this.session, resource.content, parsedSel, options);
		const raw = isRawSelector(parsedSel);
		const totalLines = raw ? resource.content.split("\n").length : splitAddressableFileLines(resource.content).length;
		const selector = resolveTailSelector(parsedSel, totalLines);
		if (selector.kind === "lines" && selector.ranges.length > 1) {
			return this.#buildInMemoryMultiRangeResult(resource.content, selector.ranges, { ...options, raw });
		}
		const { offset, limit } = selToOffsetLimit(selector);
		return this.#buildInMemoryTextResult(resource.content, offset, limit, { ...options, raw });
	}
