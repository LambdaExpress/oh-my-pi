...
/**
 * Job row that still counts as active: a running job, or a cancelled job whose
 * settlement was never observed (its cancellation cleanup is still in flight,
 * so the row must not be reported as done). The TUI keeps a block whose jobs
 * are all still active un-finalized (displaceable) so a follow-up `hub` call
...
export function runningAgentsOutsideJobs(session: ToolSession): AgentActivitySnapshot[] {
	...
	if (manager) {
		const activeJobs = manager.getAllJobs(selfId ? { ownerId: selfId } : undefined).filter(isActiveJobSnapshot);
...
export function buildJobResult(
	...
	const completed = jobResults.filter(j => j.settledAt !== undefined);
	...
	if (running.length > 0) {
		...
		for (const j of running) {
			const state = j.status === "cancelled" ? "cancellation requested · cleanup in progress" : j.label;
			lines.push(`- \`${j.id}\` [${j.type}] — ${state}`);
			if (j.progress?.text) lines.push(j.progress.text);
		}
	}
