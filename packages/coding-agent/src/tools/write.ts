...
export class WriteTool implements AgentTool<typeof writeSchema, WriteToolDetails> {
	...
	createAbortedResult(
		...
		return {
			...
			details: { xdev: { tool: targetName, mode: "execute", args, aborted: true } },
			isError: true,
		};
	}
...
function formatStreamingContent(
	...
	// Hidden head only (the collapsed path never materializes the full payload):
	...
	const hiddenVisualRows = (() => {
		...
		for (let i = 0; i < startIndex; i++) {
			...
			rows += wrapTextWithAnsi(`${gutter}${replaceTabs(hiddenLines[i] ?? "")}`.trimEnd(), contentWidth).length;
		}
		return rows;
	})();
...
export const writeToolRenderer = {
	renderCall(
		...
		if (rawPath && couldBecomeXdUrl(rawPath)) {
			...
			if (isResolutionDeviceName(xdev.name)) return renderResolutionDeviceCall(xdev.name, content, uiTheme);
			if (xdev.name === REPORT_ISSUE_DEVICE_NAME) return renderReportIssueDeviceCall(content, uiTheme);
			return renderXdevCall(xdev.name, content, options, uiTheme, options.renderContext?.resolveXdevMounted);
		}
		...
		const streamingContent = typeof rawContent === "string" ? rawContent : normalizeDisplayText(rawContent);
		const streamingCache = createRenderedStringCache();
		return framedBlock(uiTheme, width => {
			const preview = streamingContent
	...
	forceFirstResultViewportReset: true,
