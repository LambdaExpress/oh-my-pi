...
export class CollabHost {
	...
	#rejectReadOnly(action: string, fromPeer: number): void {
		this.#send({ t: "error", message: t("{action} is disabled on a read-only link", { action }) }, fromPeer);
	}
	...
	*#snapshotChunks(entries: ReplicatedEntry[]): Generator<CollabFrame> {
	...
	#handleAbort(fromPeer: number): void {
		...
		void this.#ctx.session
			...
			.then(() => {
				if (!this.#guestTrafficAllowed()) return;
				this.#ctx.session.emitNotice("info", t("{name} interrupted", { name }), "collab");
			})
	...
	async #handleModelList(fromPeer: number): Promise<void> {
		...
		this.#send({ t: "model-list", models }, fromPeer);
	}
	...
	async #handleModelChange(provider: string, id: string, fromPeer: number): Promise<void> {
		...
		if (!model) {
			this.#send({ t: "error", message: t("Model not found: {provider}/{id}", { provider, id }) }, fromPeer);
			return;
		}
		try {
			...
			this.#send({ t: "error", message: String(err) }, fromPeer);
		}
	}
	...
	#handleAgentCmd(cmd: "chat" | "kill" | "revive", agentId: string, text: string | undefined, fromPeer: number): void {
		...
		if (ref && ref.scopeId !== this.#ctx.session.getAgentScopeId()) {
			this.#send({ t: "error", message: t("agent not in this session") }, fromPeer);
			return;
		}
		...
		switch (cmd) {
			case "chat": {
				...
				if (!trimmed) {
					this.#send({ t: "error", message: t("agent {agentId}: empty chat message", { agentId }) }, fromPeer);
					return;
				}
