...
async function isolationRecoveryHint(result: SingleResult, artifactsDir: string): Promise<string> {
	...
	const hint = prompt.render(isolationRecoveryHintTemplate, {
		...
		nestedPatchPaths: nested.paths.map(nestedPatchPath => {
			const portable = portableRecoveryPath(nestedPatchPath);
			return portable === nestedPatchPath.replace(/\\/g, "/")
				? nestedPatchPath
				: `${nestedPatchPath}. Portable form: ${portable}`;
		}),
