...
interface SpawnExecutionOptions {
	...
	/** Transfers ownership of retained temporary artifacts to the async job. */
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
		...
		return manager.register(
			...
			},
			{
				...
				onProgress: (text, details) => {
					const progressDetails = (details as TaskToolDetails | undefined) ?? buildDetails();
	...
	async #runSyncSpawns(args: {
		...
		const { toolCallId, params, defaultAgent, scopeId, spawns, signal, onItemProgress } = args;
		const semaphore = this.#getSpawnSemaphore();
	...
	async #runSpawn(
		...
		try {
			const execution = await runStructuredSubagent({
				...
				...(options?.onArtifactsRetained ? { onArtifactsRetained: options.onArtifactsRetained } : {}),
