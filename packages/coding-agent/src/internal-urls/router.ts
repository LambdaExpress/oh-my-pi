...
setInternalUrlCompletionHost({
	...
	resolveCompletions: (scheme, query, context) =>
		InternalUrlRouter.instance().complete(scheme, query, {
			...context,
			// pi-tui declares the minimal `ssh://` completion shape it reads;
			// the caller always supplies its own richer SSHHost records, so the
			// narrowing here is the package boundary, not a value change.
			sshHosts: context.sshHosts as ResolveContext["sshHosts"],
		}),
});
