...
- Added an `Inject` notice that shows what the harness put into the model's context: instruction files (`AGENTS.md` and friends) with their sizes, always-apply rules, the rulebook, the skill index, memory instructions, and connected MCP servers' instructions, each with a preview of the injected body (`ctrl+o` expands). Injected skill prompts carry the same label, and under `display.foldToolRows` each notice collapses to one `Inject: AGENTS.md ×2 · Skills` row. Records are journaled, so the notice survives transcript rebuilds.
- Added an opt-in `display.foldToolRows` transcript mode that draws each activity row — every tool call (full cards, grouped reads, subagent tasks, custom/extension/MCP tools), TTSR rule injections (`TTSR: ts-set-map — …`), todo reminders, and late LSP diagnostics — as one line: `Eval: raw point-by-point inspection`, `Edit: ~/project/README.md +6 -5`, `Write: src/app.ts +42`, `Bash: bun test`. A `write xd://<device>` dispatch folds as the device operation it ran (`ADB: shell logcat -d`, `ADB: pull /sdcard/shot.png`, `atlassian/downloadJiraAttachment: CEN21-7022`) instead of restating the device URL. File targets keep their accent color, edits show `+N -M` and writes `+N` in the diff-added/removed colors, and every other tool falls back to the call's target (command, path, pattern, query). Consecutive folded rows drop the blank row the transcript normally puts between blocks, so a long tool run reads as one compact list while model replies keep their spacing. Fold and unfold the whole transcript with `app.tools.foldRows` (`Alt+Shift+O` by default) or from `/settings`, and the state survives restarts.
...
- Fixed the remaining English surfaces under zh-CN: `omp <cmd> --help` never received the translate hook, so every subcommand's help rendered English (description, usage, arguments, flags and example blocks); the settings panel left option labels/descriptions and section headings untranslated for service tiers, web-search and image providers, tiny/memory models, speech voices, compaction methods and several display modes; the slash-command picker left 22 builtin command descriptions (and the bundled `/review` + `/green` entries) in English; and the `/hotkeys` table, CLI example comments, plugin/marketplace and debug panels, tool-card section labels, and status/notice text across session, agent-hub, cleanse, worktree, commit, collab and memory backends now render Chinese.
- Fixed markdown slash commands showing their heading marker: a command file whose first line is a heading (the usual authoring form) contributed `# Foo Command` to the picker instead of `Foo Command`.
...
- Fixed orphaned session backup recovery rewriting remote or in-memory POSIX storage keys with Windows path separators.
...
- Fixed `display.collapseCompletedRuns` leaving an older expanded tool block above its collapse summary when `Alt+O` was pressed during a later streaming run; the next prompt submission now consumes the deferred non-destructive repaint and replays the authoritative collapsed transcript once.
- Fixed direct Browser `tab.id()`/`tab.ref()`/`tab.waitFor()` handle actions bypassing per-action deadlines, so stalled `click`/`type`/`fill` calls now fail with their named operation instead of consuming the whole cell timeout.
- Fixed Windows glob walks aborting on unrelated directories locked with `ERROR_SHARING_VIOLATION`, and fixed SSH downloads failing on Bun 1.3.14 when the existing local parent directory carries the ReadOnly attribute.
- Fixed `apply_patch` mistaking hunk content beginning with `--- ` or `+++ ` for unified-diff file headers, which could leave SQL comments unchanged while appending the requested replacements at EOF.
- Fixed LSP file and workspace reloads sending the rust-analyzer-private reload request to tsgo and other non-Rust servers, and fixed position-based requests querying stale open-document overlays after external edits.
- Fixed LSP absolute sibling-worktree files initializing servers at the session root, unavailable diagnostics collapsing to `all language servers failed ()`, and null rename responses being reported as successful edits.
- Fixed `task` advertising `outputSchema: false` as a valid model-generated argument even though that JSON Schema rejects every possible subagent result, which caused the first spawn attempt to fail preflight before the model retried without it.
...
- Fixed the write tool's streaming preview staying blank until `path` had streamed in: models that emit `content` before `path` (DeepSeek-family via OpenAI-compatible gateways) now show the live content preview with a placeholder path immediately, retain the full 12-line live tail, and swap the real path in when it lands.
...
- Fixed SSH uploads to Windows hosts falsely reporting failure after a commit that actually succeeded: the PowerShell commit timeout was too short for cold-start Add-Type compilation, failed commits now retry once when the identity marker was never acknowledged, and recovery confirms a committed destination by size when the proof hardlink was already cleaned up.
- Fixed completed-run collapsing after `/tree` navigation to an unfinished tool-use branch by restoring the original request anchor, so a later “continue” collapses the entire selected run instead of only the post-navigation tail.
- Fixed `apply_patch` accepting absolute local source and move paths outside its workspace-relative path contract; local patch targets now reject POSIX, Windows drive, and UNC absolute paths before any file is modified, while `ssh://` targets remain supported.
- Fixed Browser Relay forwarding every Chrome DevTools Protocol command through a fixed 20-second extension deadline, which aborted valid long-running clicks and downloads before the browser tool's requested timeout; forwarded commands now remain bounded by the browser call budget while relay control operations keep their fail-fast deadline.
- Fixed `tab.click()` and `tab.fill()` timing out on visible, enabled CSS targets by using the same abortable element-handle actionability path as accessibility references, including viewport, coverage, disabled, and read-only checks.
- Fixed LSP cold starts treating a language server's clean exit during initialization as a cached permanent failure, diagnostics for absolute external-worktree globs starting the server at the session workspace instead of the target project root, and document-symbol lines pointing at decorator ranges that could not be reused by position-based actions.
- Fixed GitHub code search failing immediately on transient `unexpected EOF` and TLS handshake errors; read-only search API calls now use bounded, abortable retries without repeating deterministic failures or write operations.
...
- Fixed completed background SSH transfers staying pinned in the transfer HUD at 100% when the result was consumed via `hub wait`/`hub jobs`: those calls acknowledge the async delivery (so no `async-result` follow-up is injected), and the HUD-clear wired for the pre-merge `job` tool name never fired for `hub`, leaving the settled progress bar rendered below the transcript forever. Hub job-style results now mark their settled transfer ids as persisted, and `readPersistedJobIds` accepts both `id` (hub snapshots) and `jobId` (async-result details) keys.
