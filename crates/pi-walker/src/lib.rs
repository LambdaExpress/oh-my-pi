	...
	H: Fn() -> std::result::Result<(), E> + Sync + 'scope,
{
	...
	for index in 0..scratch.entries.len() {
		...
		let name = DirScratch::name(&scratch, entry);
...
impl DirScratch {
	...
	#[cfg(unix)]
	fn name<'a>(scratch: &'a Self, entry: &DirEntryRecord) -> &'a OsStr {
		OsStr::from_bytes(&scratch.name_bytes[entry.name_start..entry.name_start + entry.name_len])
	}

	#[cfg(not(unix))]
	fn name<'a>(_scratch: &Self, entry: &'a DirEntryRecord) -> &'a OsStr {
		&entry.name
	}
}
