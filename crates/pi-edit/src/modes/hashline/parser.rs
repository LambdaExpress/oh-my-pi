...
impl Executor {
	...
	fn handle_literal(&mut self, text: &str, line_num: u32) -> Result<(), ParseFailure> {
		if let Some(pending) = self.pending.as_ref() {
			let recovered_operation = Tokenizer::new().tokenize(text, line_num);
			let has_trailing_blank = !pending.deferred_blanks.is_empty()
				|| pending
					.payloads
					.last()
					.is_some_and(|payload| payload.text.trim().is_empty());
			if has_trailing_blank
				&& matches!(&recovered_operation, Token::OpBlock {
					target: BlockTarget::Cut { .. } | BlockTarget::CutBlock { .. },
					..
				}) {
				return self.feed(recovered_operation);
			}
		}
