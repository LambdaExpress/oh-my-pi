...
impl EditStore {
	...
	/// Record normalized text under an opaque canonical key. Unlike
	/// [`Self::record_file`], this never resolves the key as a filesystem path.
	pub fn record_key(&self, key: &str, text: &str, seen_lines: Option<&[u32]>) -> String {
		self.record(Path::new(key), text, seen_lines)
	}
	...
	/// Union displayed lines into a snapshot addressed by an opaque canonical
	/// key without filesystem canonicalization.
	pub fn record_seen_lines_key(&self, key: &str, hash: &str, lines: &[u32]) {
		self.record_seen_lines(Path::new(key), hash, lines);
	}
