...
use crate::{
	...
	notebook,
	path_policy::{PathPolicy, VirtualResource, canonical_key},
...
pub struct WriteRequest {
	...
	pub display:      String,
	pub op:           FileOp,
	/// Absolute local destination or canonical URL for a rename. The host
	/// writes `content` there and deletes `absolute`.
...
impl Session {
	...
	/// Add or refresh one host-materialized internal target. A refreshed
	/// resource invalidates cached previews and is reread on the next pass.
	pub fn preload_virtual_resource(&mut self, resource: VirtualResource) -> EditResult<()> {
		self.files.preload_virtual_resource(resource)?;
		self.generation += 1;
		self.final_pass_done = false;
		Ok(())
	}
	...
	pub async fn apply(
		...
		for file in &staged {
			...
			if let Some(destination) = &file.move_to {
				self.files.enforce_move(
					&Resolved { absolute: file.absolute.clone(), display: file.display.clone() },
					destination,
				)?;
			}
		}
