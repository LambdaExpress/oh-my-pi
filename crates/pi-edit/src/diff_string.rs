...
/// Strip pre-hunk wrapper/metadata lines and trailing blank rows from a diff
/// body.
pub fn normalize_diff(diff: &str) -> String {
	...
	let mut before_first_hunk = true;
	lines.retain(|line| {
		let trimmed = line.trim();
		if before_first_hunk && trimmed.starts_with("@@") {
			before_first_hunk = false;
		}
		!before_first_hunk
			|| (is_diff_content_line(line)
				|| (!is_patch_wrapper_line(trimmed)
					&& !matches_trimmed_prefix(trimmed, &DIFF_METADATA_PREFIXES)))
	});
	lines.join("\n")
}
