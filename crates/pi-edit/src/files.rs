...
/// Default [`FileSource`] backed by `std::fs` for local paths and by explicit
/// host materializations for internal URLs.
pub struct FileCache {
	policy:        PathPolicy,
	reads:         HashMap<PathBuf, (Stamp, Arc<FileRead>)>,
	virtual_reads: HashMap<PathBuf, Arc<FileRead>>,
	virtuals:      VirtualResources,
...
impl FileCache {
	pub fn new(policy: PathPolicy) -> Self {
		Self {
			policy,
			reads: HashMap::new(),
			virtual_reads: HashMap::new(),
			virtuals: VirtualResources::default(),
			resolutions: HashMap::new(),
		}
	}

	/// Add or refresh one host-materialized internal target.
	pub fn preload_virtual_resource(&mut self, resource: VirtualResource) -> EditResult<()> {
		let canonical = PathBuf::from(&resource.canonical_url);
		self.virtuals.preload(resource)?;
		self.virtual_reads.remove(&canonical);
		self.resolutions.clear();
		Ok(())
	}

	/// Enforce move boundaries using the exact virtual resources admitted to
	/// this cache.
	pub fn enforce_move(&self, from: &Resolved, to: &Resolved) -> EditResult<()> {
		self.policy.enforce_move(from, to, &self.virtuals)
	}

	fn read_virtual(&mut self, resolved: &Resolved) -> EditResult<Option<Arc<FileRead>>> {
		let Some(resource) = self.virtuals.by_absolute(&resolved.absolute).cloned() else {
			return Ok(None);
		};
		let Some(raw) = resource.content else {
			return Ok(None);
		};
		if let Some(read) = self.virtual_reads.get(&resolved.absolute)
		...
		if let Some(message) = self
			.policy
			.auto_generated_message(&resolved.display, &raw.as_bytes()[..raw.len().min(1024)])
		{
		...
		self
			.virtual_reads
			.insert(resolved.absolute.clone(), Arc::clone(&read));
		Ok(Some(read))
	}

	fn read_resolved(&mut self, resolved: &Resolved) -> EditResult<Option<Arc<FileRead>>> {
		if self.virtuals.by_absolute(&resolved.absolute).is_some() {
			return self.read_virtual(resolved);
		}
...
impl FileSource for FileCache {
	...
	fn resolve(&mut self, authored: &str, must_exist: bool) -> EditResult<Resolved> {
		...
		let mut resolved = self.policy.resolve_with_virtual(authored, &self.virtuals)?;
		if must_exist
	...
	fn exists(&mut self, absolute: &Path) -> bool {
		if let Some(resource) = self.virtuals.by_absolute(absolute) {
			return resource.content.is_some();
		}
