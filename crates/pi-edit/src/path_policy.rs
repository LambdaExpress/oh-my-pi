...
/// One host-materialized internal edit target. URLs are identities only:
/// Rust never resolves them or performs protocol I/O.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VirtualResource {
	/// Stable, credential-free key supplied by the protocol handler.
	pub canonical_url: String,
	/// Credential-free URL shown to the model and returned to the host.
	pub display_url:   String,
	/// Existing UTF-8 text, or `None` when the target was explicitly observed
	/// as missing.
	pub content:       Option<String>,
}

/// Internal targets explicitly admitted for one edit session.
#[derive(Debug, Clone, Default)]
pub struct VirtualResources {
	entries: Vec<VirtualResource>,
}

impl VirtualResources {
	/// Add or refresh a host-materialized target.
	pub fn preload(&mut self, resource: VirtualResource) -> EditResult<()> {
		let canonical_identity = virtual_url_identity(&resource.canonical_url)?;
		let display_identity = virtual_url_identity(&resource.display_url)?;
		if canonical_identity != display_identity {
			return Err(EditError::apply(format!(
				"Virtual edit resource canonical and display URLs must use the same protocol \
				 authority: {}",
				resource.display_url
			)));
		}
		if self.entries.iter().any(|existing| {
			existing.canonical_url != resource.canonical_url
				&& (existing.canonical_url == resource.display_url
					|| existing.display_url == resource.canonical_url
					|| existing.display_url == resource.display_url)
		}) {
			return Err(EditError::apply(format!(
				"Virtual edit resource URL is already registered for another target: {}",
				resource.display_url
			)));
		}
		if let Some(existing) = self
			.entries
			.iter_mut()
			.find(|existing| existing.canonical_url == resource.canonical_url)
		{
			*existing = resource;
		} else {
	...
	fn resolve(&self, url: &str) -> Option<&VirtualResource> {
		self
			.entries
			.iter()
			.find(|resource| resource.canonical_url == url || resource.display_url == url)
	}

	pub(crate) fn by_absolute(&self, absolute: &Path) -> Option<&VirtualResource> {
		let absolute = absolute.to_str()?;
		self
			.entries
			.iter()
			.find(|resource| resource.canonical_url == absolute)
	}
}
...
impl PathPolicy {
	/// Resolve through the local policy or an explicitly preloaded internal
	/// target. Internal URLs that were not preloaded fail closed.
	pub fn resolve_with_virtual(
		&self,
		authored: &str,
		...
		{
			let resource = resources.resolve(&normalized).ok_or_else(|| {
				EditError::apply(format!(
					"Internal edit target was not preloaded for this session: {display}"
				))
			})?;
			return Ok(Resolved {
				absolute: PathBuf::from(&resource.canonical_url),
				display:  resource.display_url.clone(),
			});
		}
	...
	/// Reject moves that would cross the host/filesystem boundary or change
	/// the protocol authority. The session calls this for every staged move
	/// before invoking the first writer callback.
	pub fn enforce_move(
		&self,
		from: &Resolved,
		to: &Resolved,
		...
		let source = resources.by_absolute(&from.absolute);
		let destination = resources.by_absolute(&to.absolute);
		match (source, destination) {
			(None, None) => Ok(()),
			(Some(_), None) | (None, Some(_)) => Err(EditError::apply(format!(
				"Cannot move between an internal URL and a local filesystem path: {} -> {}",
				from.display, to.display
			))),
			(Some(source), Some(destination)) => {
				if virtual_url_identity(&source.canonical_url)?
					!= virtual_url_identity(&destination.canonical_url)?
				{
					return Err(EditError::apply(format!(
						"Cannot move an internal URL across protocol authorities: {} -> {}",
						from.display, to.display
					)));
				}
				Ok(())
...
pub fn canonical_key(absolute: &Path) -> PathBuf {
	if absolute.to_str().is_some_and(is_internal_url) {
...
fn virtual_url_identity(value: &str) -> EditResult<(&str, &str)> {
	let Some((scheme, rest)) = value.split_once("://") else {
		return Err(EditError::apply(format!(
			"Virtual edit resource must use an internal URL: {value}"
		)));
	};
	if !INTERNAL_PREFIXES
		.iter()
		.any(|prefix| prefix.strip_suffix("://") == Some(scheme))
		|| matches!(scheme, "local" | "vault")
	{
		return Err(EditError::apply(format!(
			"Virtual edit resource must use a host-backed internal URL: {value}"
		)));
	}
	...
	{
		return Err(EditError::apply("Virtual edit resource URL must not contain credentials"));
	}
	Ok((scheme, authority))
}
