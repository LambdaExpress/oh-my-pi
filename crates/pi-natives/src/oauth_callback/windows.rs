...
pub(super) fn restore(context: &Context, snapshot: &Snapshot) -> Result<()> {
	...
	if marker_current.as_ref() != Some(marker_owned) {
		bail!(
	...
	context.check()?;
	if read_value(&layout.root, MARKER_NAME)?.as_ref() != Some(marker_owned) {
...
impl Layout {
	fn new(scheme: &str) -> Self {
		...
		Self {
			root:   root.clone(),
			keys:   [root.clone(), shell, open, command.clone()],
			values: [
...
fn validate_snapshot(context: &Context, snapshot: &Snapshot) -> Result<Layout> {
	...
	if snapshot.keys.len() != layout.keys.len() || snapshot.values.len() != layout.values.len() {
		bail!("invalid Windows OAuth callback registry snapshot size");
	}
	for (index, (entry, expected)) in snapshot.keys.iter().zip(layout.keys.iter()).enumerate() {
...
fn decode_reg_sz(value: &RegValue<'_>) -> Option<OsString> {
	...
	let (units, _) = value.bytes.as_chunks::<2>();
	let mut units = units
		.iter()
...
fn notify_association_changed() {
	// SAFETY: Windows requires `SHCNE_ASSOCCHANGED` to be sent alone with
	// `SHCNF_IDLIST` and both item pointers null. The API does not retain the
	// pointers or take ownership of any caller memory.
	unsafe {
		SHChangeNotify(
...
fn effective_command(scheme: &str) -> Result<OsString> {
	...
	// SAFETY: `association` is NUL-terminated and remains alive for the call.
	// `output` contains `length` writable `u16` elements, and `length` is a
	// valid in/out pointer initialized to that capacity. `pszExtra` may be null
	// for `ASSOCSTR_COMMAND`; the API neither retains nor takes ownership of any
	// supplied pointer.
	let result = unsafe {
		AssocQueryStringW(
...
mod tests {
	...
	fn recovery_preserves_external_command_and_retains_ownership_marker() {
		...
		write_value(&layout.keys[3], DEFAULT_VALUE, &external).unwrap();
