...
use pi_edit::{
	...
	path_policy::{VirtualResource, canonical_key},
...
/// One internal edit target materialized by the host. Both URLs must be
/// credential-free; `content: null` explicitly preloads a missing target.
#[napi(object)]
pub struct EditVirtualResource {
	pub canonical_url: String,
	pub display_url:   String,
	pub content:       Option<String>,
}

impl From<EditVirtualResource> for VirtualResource {
	fn from(resource: EditVirtualResource) -> Self {
		Self {
...
pub struct EditWriteRequest {
	/// Absolute local path or canonical URL for a host-materialized target.
	pub path:         String,
	/// Local authored path or host-provided display URL.
...
impl EditStore {
	...
	/// Record text under an opaque canonical key without treating the key as
	/// a local path.
	#[napi]
	pub fn record_snapshot_for_key(
		&self,
		...
		self
			.inner
			.record_key(&canonical_key, &normalize_to_lf(body), seen_lines.as_deref())
	}
	...
	/// Merge displayed lines into a snapshot addressed by an opaque canonical
	/// key.
	#[napi]
	pub fn record_seen_lines_for_key(&self, canonical_key: String, tag: String, lines: Vec<u32>) {
		self
			.inner
			.record_seen_lines_key(&canonical_key, &tag, &lines);
	}
	...
	/// Merge hashline-formatted displayed lines into a snapshot addressed by
	/// an opaque canonical key.
	#[napi]
	pub fn record_seen_lines_from_body_for_key(
		&self,
		canonical_key: String,
		tag: String,
		body: String,
...
impl EditSession {
	...
	/// Add or refresh one host-materialized internal target. Await completion
	/// before pushing arguments that reference the resource.
	#[napi]
	pub async fn preload_virtual_resource(&self, resource: EditVirtualResource) -> Result<()> {
		if self.shared.closed.load(Ordering::Acquire) {
			return Err(reason("Cannot preload a closed edit session"));
		}
		...
		session
			.preload_virtual_resource(resource.into())
			.map_err(reason)?;
		drop(session);
