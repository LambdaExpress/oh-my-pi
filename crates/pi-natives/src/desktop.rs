...
fn move_mouse(_input: &mut Enigo, x: i32, y: i32) -> CoreResult<()> {
	...
	// SAFETY: GetSystemMetrics only reads process-global system metric state and
	// accepts metric identifiers by value; these calls pass no pointers.
	...
	// SAFETY: `event` is fully initialized, the count matches the single valid
	// input reference, and `cbSize` is the exact size of `INPUT`.
