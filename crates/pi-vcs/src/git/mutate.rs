...
impl GitRepo {
	...
	pub fn stage_files(&self, files: &[String]) -> Result<()> {
		...
		for path in selected {
			stage_one(&repo, self.root(), &mut index, &mut filter, &path)?;
		}
		index.sort_entries();
		index
			.write(INDEX_WRITE)
...
fn precomposed(path: &str) -> Cow<'_, str> {
	gix_utils::str::precompose(Cow::Borrowed(path))
}
...
#[cfg(not(unix))]
const fn same_worktree_file(_: &fs::Metadata, _: &fs::Metadata) -> bool {
	false
}
...
fn checkout_tree(
	...
	let conflicts = checkout_conflicts(repo, &current, &target)?;
...
fn checkout_conflicts(
	...
	let (mut filter, _) = repo
		.filter_pipeline(None)
		.map_err(|err| Error::backend("git checkout", err))?;
	...
	for entry in current.entries() {
		...
		if worktree_id(repo, &mut filter, current, &path)? != Some(entry.id) {
			conflicts.push(path);
		}
	}
	conflicts.sort();
...
fn worktree_id(
	...
	let full = filter
		.repo
		.workdir()
		.unwrap_or_else(|| filter.repo.git_dir())
		.join(path);
...
fn tracked_worktree_dirty(repo: &GitRepo) -> Result<bool> {
	...
	let (mut filter, _) = gix
		.filter_pipeline(None)
		.map_err(|err| Error::backend("git worktree remove", err))?;
	for entry in index.entries() {
		if worktree_id(&gix, &mut filter, &index, entry.path(&index).to_str_lossy().as_ref())?
			!= Some(entry.id)
		{
			return Ok(true);
		}
