...
impl GitRepo {
	...
	pub fn stash_try_pop(&self, reinstate_index: bool) -> Result<bool> {
		...
		let (mut filter, _) = repo
			.filter_pipeline(None)
			.map_err(|err| Error::backend("git stash pop", err))?;
		for (path, entry) in &untracked {
			write_worktree_entry(self, path, entry, &repo, &mut filter)?;
		}
...
fn augment_patch_sources(
	...
	for patch in patches {
		...
		if let Some((bytes, mode)) =
			read_worktree_entry(repo, &mut filter, &filter_index, path, mode)?
		{
			let id = gix_repo
				.write_blob(bytes)
				.map_err(|err| Error::backend("git hash patch source", err))?
...
fn tracked_worktree_map(
	...
	for (path, entry) in index {
		if let Some((bytes, mode)) =
			read_worktree_entry(repo, &mut filter, &filter_index, path, entry.mode)?
		{
			let id = gix_repo
				.write_blob(bytes)
				.map_err(|err| Error::backend("git hash worktree blob", err))?
...
fn untracked_worktree_map(
	...
	for item in walk {
		...
		if let Some((bytes, mode)) =
			read_worktree_entry(repo, &mut filter, &filter_index, &path, Mode::FILE)?
		{
			let id = gix_repo
				.write_blob(bytes)
				.map_err(|err| Error::backend("git hash untracked blob", err))?
...
fn read_worktree_entry(
	...
	let mut input = filter
		.convert_to_git(file, Path::new(path), index)
		.map_err(|err| Error::backend("git clean filter", err))?;
...
fn write_patch_worktree(
	...
	let (mut filter, _) = gix_repo
		.filter_pipeline(None)
		.map_err(|err| Error::backend("git apply smudge filter", err))?;
	for patch in patches {
		...
		if let Some(target) = target {
			...
			write_worktree_entry(repo, target, entry, &gix_repo, &mut filter)?;
		}
	}
	Ok(())
}
...
fn write_worktree_map(
	...
	let (mut filter, _) = gix_repo
		.filter_pipeline(None)
		.map_err(|err| Error::backend("git worktree smudge filter", err))?;
	...
	for (path, entry) in next {
		if previous.get(path) != Some(entry) || !repo.root().join(path).exists() {
			write_worktree_entry(repo, path, entry, &gix_repo, &mut filter)?;
		}
	}
	Ok(())
}
...
fn write_worktree_entry(
	...
	let mut output = filter
		...
		.map_err(|err| Error::backend("git smudge filter", err))?;
	if let Some(bytes) = output.as_bytes() {
		fs::write(&absolute, bytes)?;
	} else {
...
mod tests {
	...
	#[test]
	fn patch_apply_honors_autocrlf_in_both_directions() {
		let temp = init(&[("file.txt", b"one\ntwo\n")]);
		git(temp.path(), &["config", "core.autocrlf", "true"]);
		fs::remove_file(temp.path().join("file.txt")).expect("remove worktree file");
		git(temp.path(), &["checkout", "--", "file.txt"]);
		assert_eq!(fs::read(temp.path().join("file.txt")).expect("checkout CRLF"), b"one\r\ntwo\r\n");

		fs::write(temp.path().join("file.txt"), b"one\r\nchanged\r\n").expect("edit CRLF file");
		let patch = git(temp.path(), &["diff", "--binary"]);
		fs::remove_file(temp.path().join("file.txt")).expect("remove edited file");
		git(temp.path(), &["checkout", "--", "file.txt"]);
		...
		assert_eq!(
			fs::read(temp.path().join("file.txt")).expect("patched CRLF"),
			b"one\r\nchanged\r\n"
		);

		repository
			.apply_patch(&patch, &ApplyOptions { reverse: true, ..ApplyOptions::default() })
			.expect("reverse CRLF patch");
		assert_eq!(fs::read(temp.path().join("file.txt")).expect("reversed CRLF"), b"one\r\ntwo\r\n");
