...
mod proc_snapshot {
	...
	impl ProcInfo {
		...
		pub const fn terminal_group_id(&self) -> Option<i32> {
			None
		}
...
#[cfg(not(unix))]
const fn resolve_terminal(_value: &str) -> Option<u64> {
	None
}
...
#[cfg(target_os = "windows")]
const fn ps_total_memory_bytes() -> Option<u64> {
	None
}
...
#[cfg(not(unix))]
const fn ps_user_name(_uid: u32) -> Option<String> {
	None
}
...
#[cfg(not(unix))]
const fn ps_group_name(_gid: u32) -> Option<String> {
	None
}
...
#[cfg(not(unix))]
const fn ps_page_size() -> Option<u64> {
	None
}
